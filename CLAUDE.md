...
2. Game UI screens (nie lil-gui) → [plans/2026-08-07--005--game-ui-screens.md](docs/plans/2026-08-07--005--game-ui-screens.md) (`in progress`: pause menu + Character (imię gracza) done — `src/ui/createPauseMenu.ts`; World config / Notes / NPC dialog open). UI stack: [plans/2026-08-09--046--vue-tailwind-ui-stack.md](docs/plans/2026-08-09--046--vue-tailwind-ui-stack.md) (`in progress` — **Faza 0 done**: Vue 3 + Tailwind v4 + `lucide-vue-next` w zależnościach, `#vue-ui` mount w `createApp.ts` przez dynamiczny `import()`, PoC komponent `src/ui-vue/App.vue`, `vue-tsc` w `npm run build`; Fazy 1+ — migracja realnych ekranów — nie ruszone). Kolejny konsument: [plans/2026-08-09--048--npc-dialogues-v2.md](docs/plans/2026-08-09--048--npc-dialogues-v2.md) (`todo` — menu rozmowy NPC ma być pierwszym realnym ekranem w Vue, zależy tylko od Fazy 0 wyżej, nie od migracji innych ekranów)
