...
**Update (2026-08-15):** Fixed the `waitForChunks` idle-drain half of this — see [issue 032, "Fix (2026-08-15)"](./2026-08-15--032--mobile-black-world-screen.md#fix-2026-08-15--twardy-budżet-czasu-na-idle-drain-finalizacji-chunków) for the change and re-audit. The `080fd3f` `buildInstancedProps()` half is **not** fixed — re-checked and it turned out to be a narrower change than first read: `createPropYieldGate` still gates the per-item placement-collection loops (houses, gardens, plaza bushes); only the final per-category "build the `InstancedMesh` from already-collected placements" step became one synchronous call, which is cheap CPU/matrix work, not GLB-clone+material-tint. Still a secondary, unmeasured contributor — remains open here.
