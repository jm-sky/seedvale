...
- `createCaves` / Cave V2 — streamed walk-in interiors (`src/world/caves/`); the retained `CaveHeightfieldRepresentation` is the single spatial authority (`world-terrain-019`): presentation mesh assembled on the main thread with a real terrain mouth cutout owned by `ChunkManager`; `queryGround` / `sampleFloor` / `sampleCeiling` (player ground, neutral hit + underground-miss hysteresis in `caveGroundQuery.ts`), `occupancyAt` / `contains` (strict, stateless — camera boom, swim eligibility, torch), `resolveHorizontal` (entity-neutral wall containment used by `PlayerController` after ordinary colliders) and `queryInterior` (hysteretic player-position interior flag) all read it via `caveHeightfieldQuery.ts`. No cave colliders, no SDF. Camera mouth-exit from a portal or surface-clipped hood origin; stable-interior boom never falls back to the outdoor heightfield and shortens instead of parking in the mouth throat. Browser verification of the containment / camera migration pending by the User. The `?caveHeightfieldTest` harness is debug-only and imports the production helpers. Two archetypes (`world-terrain-020` Stage A): `natural` and the longer `adventure` recipe; assignment (home guarantee + 15% roll + adventure→natural fallback) is pure in `caves/caveArchetype.ts` and exposed as `Caves.archetypeOf`.
