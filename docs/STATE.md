...
- `createCaves` / Cave V2 — streamed walk-in interiors (`src/world/caves/`); presentation is production SDF; gameplay ground is the SDF column index (`queryGround`); wall colliders and camera occupancy are strict occupancy derived from that index (`occupancyAt`, `caveSdfColliders.ts`); `queryInterior` is the hysteretic player-position interior flag (cave ambience / rain mute). Open-sky is portal-only (not SDF surface-clip); camera mouth-exit from a portal or surface-clipped hood origin; stable-interior boom never falls back to the outdoor heightfield and shortens instead of parking in the mouth throat. Hillside doorway / terrain-hole presentation is still an open B3 leftover (manual verification still required).
