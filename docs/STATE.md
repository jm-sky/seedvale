...
- Wait / camp rest / town rest / tent rest exist. Quick Actions gate town rest on `nearTown`. Esc during rest aborts the skip before opening pause. Rest fully restores vigor/stamina (plan 106). Busy channels (dig/chop/mine/bury/harvest/ignite/cook/destroy-spawner) are short real-time overlays (seconds, not minutes); harvest/ignite/cook/destroy-spawner show a progress bar and can be Esc-cancelled. Harvest pins the corpse so it cannot despawn mid-channel.
