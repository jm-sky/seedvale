...
Predator/prey/livestock ecosystem built on one shared `AnimalAgent` class — wild fauna, livestock, and rats are the same class distinguished only by ownership/registration. Behaviour is a fixed-priority threat/social-override table on top, with hunger/thirst-seeking resolved ad hoc underneath (structurally different from NPC's unified three-producer pressure competition). Persistence is four-tier: livestock persists per individual (an explicit pre-save capture step), spawner lifecycle persists thin, wild individuals are unpersisted with only the population deterministically reconstructed, and settlement rats persist per individual (plus a settlement-owned infestation record for storage damage and nest destruction). Rats' food-driven carrying capacity is independent of dogs; dogs hunt rats physically and reduce nest-gated infestation replenishment.
