...
- Prey spawners (cave / thicket) with respawn; placement rejects roads, coast/beach, and other spawn points; village avoidance uses each settlement’s real footprint radius. A `wolfDen` spawner (plan 093 Etap E) reuses the same shape/prop for a one-time, non-respawning wolf pack (quest target, not a nuisance spawner). Cave/thicket start at `maxPreyCount` (deer 3 / stag 2) and restock in **game-days** (plan 139): `respawnIntervalDays` 1 / 2, empty site `×2`, catch-up from `elapsedDays` so time-skip counts, first frame after load is `dayDelta = 0`; `wolfDen` stays `Infinity`. Spawn-point population lifecycle (plan 125): each managed `PreySpawner` (cave/thicket, not `wolfDen`) carries a stable `id` (`settlementId:type`), a `SpawnPointState` (`active`/`depleted`/`disabled`/`recovering`) and `deathsThisCycle` — `AnimalAgent.spawnPointId` (set only for spawner-created animals) routes each death through the existing `onDeath` hook to the owning spawner, no double-counting; `>50%` of `maxPreyCount` deaths moves it to `depleted` (no more respawn); `[E] Zniszcz` there is a 5 s busy channel (plan 137, 4 branches spent on complete) that moves it to `disabled`, lights a spectacle `PlacedFires` `pit` (`habitatBurn`: not `[E]`-lit, not saved, ring despawns shortly after the ~5 min burn), tints the spawner prop near-black (mesh stays) and applies `ChunkManager.scorchTerrain()` (r≈7 charcoal vertex tint + `roadTint` grass fade); after `RECOVERY_DAYS` (21) with ≥2 live same-kind animals nearby it returns to `active` with counters reset — checked at most once per in-game day, not per frame. Lifecycle state persists (`SaveData` v17 `spawnPoints`, `AnimalSpawner.ts`'s `snapshotSpawnPointState`/`restoreSpawnPointState`) across both a real save/load and an in-session `rebuildWorldBundle()`; a restored `depleted`/`disabled`/`recovering` point does not respawn its cave/thicket population, and a restored `disabled`/`recovering` point reapplies the burned prop tint + terrain scorch.
...
- Tools in the world: shovel (dig/level soil), axe and battle_axe (multi-stage tree harvest; `isChopTool`), pickaxe (ore + mountain rock), knife and damascus_knife (melee + corpse meat harvest; `isHarvestKnife`), tent (place/rest/pack), garden pitchfork/sickle. High-quality melee (plan 160): damascus knife/short/long, masterwork sword, obsidian sword, battle axe — Kupiec or quest rewards, no world spawn; held/drop GLBs wired (`items/*.glb`). Damascus blades are baked teal/navy banded steel (not gray); obsidian is volcanic-glass purple/black. Starting kit: knife, firestarter, blanket when missing.
