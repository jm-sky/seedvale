...
Procedurally chunked, streamed terrain with instanced vegetation/rocks, tree lifecycle, weather/seasons as deterministic functions of `(seed, elapsedDays)`, mountains, and rivers with their own hydrology/geometry. Settlement cemeteries are assignment-driven landmarks (`cemeteryAssignment.ts` / `cemeteryPlacement.ts`): each settlement has exactly one active cemetery (dedicated, or shared between two nearby `SM`s); rare abandoned wilderness cemeteries are a separate low-chance path and never become a settlement's burial destination. Ocean, lakes, day/night, fog and post-processing are implemented. Short-lived environmental blood traces are created directly from player/NPC/animal damage entry points and rendered via one capacity-fixed instanced mesh, resynced to whatever traces are within the streamed-terrain radius; not persisted, carried across an in-session `WorldBundle` rebuild the same way other player-positioned world objects are. Walk-in caves are Cave V2 mid-migration to the **heightfield representation** (`createCaves()` / `src/world/caves/`, plan `world-terrain-019`): player cave ground / floor / ceiling (`Caves.queryGround` / `sampleFloor` / `sampleCeiling`) read the `CaveHeightfieldRepresentation` through `caveHeightfieldQuery.ts` — the same floor the mesh renders — with the representation-neutral hit/hysteresis contract in `caveGroundQuery.ts`; strict containment (`Caves.contains`) still uses a derived SDF column index (`caveSdfQuery.ts`); body collision and third-person camera occupancy use the same index's strict occupancy (`caveSdfColliders.ts` / `Caves.occupancyAt`); `Caves.queryInterior` is the hysteretic interior flag for cave ambience and rain mute. Approach/mouth carve intervals are open-sky (no rock `maxY`); SDF occupancy stops at the mouth plane so the approach is portal-only. Doorway aperture, sides, lip and hood are derived from `CaveTopology.entrance` (`mouthCarve.ts` / `caveSdfField.ts`) before mesh extraction. Presentation is the **heightfield mesh** (`world-terrain-019` Milestone B): each accepted topology also builds a `CaveHeightfieldRepresentation` from deterministic base surface minus `mouthCarveDepth`; `caveHeightfieldMesh.ts` / `caveHeightfieldPresentation.ts` assemble the welded floor/ceiling mesh, mouth underside mask and contour-driven rocks synchronously on the main thread, streamed by the unchanged 55/80 m controller through a nearest-first queue; collider registration does not wait for the mesh. The cave mouth is a real terrain hole: `createCaves()` registers a per-cave `TerrainCutout` (`caveTerrainCutout.ts`, contour = `mouthOpeningAt`) with `ChunkManager.registerTerrainCutouts()`, which applies it on every chunk mesh build/rebuild (`terrainCutout.ts` marching-squares clip of the indexed chunk grid; survives unload/reload, dig, scorch, prepare; not a persisted modification). Strict occupancy, interior, collision and camera remain SDF until Milestone D; the SDF extraction worker / `sdfCaveMesh.ts` / `largeCaveVisual.ts` are no longer called by production and await Milestone E cleanup. `topologyToCaveDefinition` / `CaveVolume` remain catalog/streaming leftovers. The isolated `?caveHeightfieldTest` harness reuses the production representation and mesher without booting `WorldBundle`.
...
- `createCaves` / Cave V2 — streamed walk-in interiors (`src/world/caves/`); presentation is the heightfield mesh assembled on the main thread (`world-terrain-019` B) with a real terrain mouth cutout owned by `ChunkManager`; player cave ground / floor / ceiling is the same heightfield (`queryGround` / `sampleFloor` / `sampleCeiling` via `caveHeightfieldQuery.ts`, neutral hit + underground-miss hysteresis in `caveGroundQuery.ts`); wall colliders, `contains` and camera occupancy are strict occupancy derived from the transitional SDF column index (`occupancyAt`, `caveSdfColliders.ts`) and register on 55 m relevance without waiting for the mesh; `queryInterior` is the hysteretic player-position interior flag (cave ambience / rain mute). Camera mouth-exit from a portal or surface-clipped hood origin; stable-interior boom never falls back to the outdoor heightfield and shortens instead of parking in the mouth throat. The real terrain hole landed in Milestone B; the SDF-vs-rendered-floor player sink was removed by the post-B ground migration (browser verification of both still pending by the User). The `?caveHeightfieldTest` harness is debug-only and does not change this production path.
