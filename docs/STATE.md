...
- Village gardens scale (plan 077): ~1 garden unit per 3 houses packed into S/M/L beds; garden clearings keep trees out; `landmarks.gardens[]` with primary `garden` for farmers. Garden centers stay outside the plaza disk (`plazaCoreRadius + gardenPlotRadius`, plan 095) in home and non-home villages. Garden mesh is `crops.glb` (tiled for M/L); `createGarden` is fallback. Settlements with `foodSourceType === 'field'` also place `farm.glb` at the `field` landmark (`createWheatField` fallback) (plan 099).
