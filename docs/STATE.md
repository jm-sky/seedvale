...
- `createCaves` / Cave V2 — streamed walk-in interiors (`src/world/caves/`); presentation is the heightfield mesh assembled on the main thread (`world-terrain-019` B) with a real terrain mouth cutout owned by `ChunkManager`; gameplay ground is the SDF column index (`queryGround`); wall colliders and camera occupancy are strict occupancy derived from that index (`occupancyAt`, `caveSdfColliders.ts`) and register on 55 m relevance without waiting for the mesh; `queryInterior` is the hysteretic player-position interior flag (cave ambience / rain mute). Open-sky is portal-only (not SDF surface-clip); camera mouth-exit from a portal or surface-clipped hood origin; stable-interior boom never falls back to the outdoor heightfield and shortens instead of parking in the mouth throat. Hillside doorway / terrain-hole presentation and a remaining interior player-Y surface snap (~16 m in on seed `1136726869`) are open B3 leftovers (manual verification still required). The `?caveHeightfieldTest` harness is debug-only and does not change this production path.
