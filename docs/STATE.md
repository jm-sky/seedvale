...
- **House Builder** (`src/settlement/houseBuilder.ts`, plan 111) assembles MegaKit plaster cottages (4×4 / 6×4) and medium farmsteads (6×6 / 8×6) from that catalog (native metres, cap roofs, instanced static parts, hinge doors). Wired into `buildSettlementProps()`; `houseCatalog.ts` remains for Asset Browser / procedural fallback. Browser and `?perf=1` verification are still open.
