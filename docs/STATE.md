...
Procedurally chunked, streamed terrain with instanced vegetation/rocks, tree lifecycle, weather/seasons as deterministic functions of `(seed, elapsedDays)`, mountains, and rivers with their own hydrology/geometry. Settlement cemeteries are assignment-driven landmarks (`cemeteryAssignment.ts` / `cemeteryPlacement.ts`): each settlement has exactly one active cemetery (dedicated, or shared between two nearby `SM`s); rare abandoned wilderness cemeteries are a separate low-chance path and never become a settlement's burial destination. Ocean, lakes, day/night, fog and post-processing are implemented. Short-lived environmental blood traces are created directly from player/NPC/animal damage entry points and rendered via one capacity-fixed instanced mesh, resynced to whatever traces are within the streamed-terrain radius; not persisted, carried across an in-session `WorldBundle` rebuild the same way other player-positioned world objects are. Walk-in caves are Cave V2 (production SDF interiors, `createCaves()` / `src/world/caves/`); gameplay ground/containment uses a derived SDF column index (`caveSdfQuery.ts`); body collision and third-person camera occupancy use the same index's strict occupancy (`caveSdfColliders.ts` / `Caves.occupancyAt`); `Caves.queryInterior` is the hysteretic interior flag for cave ambience and rain mute. Approach/mouth carve intervals are open-sky (no rock `maxY`); SDF occupancy stops at the mouth plane so the approach is portal-only. Doorway aperture, sides, lip and hood are derived from `CaveTopology.entrance` (`mouthCarve.ts` / `caveSdfField.ts`) before mesh extraction. Presentation meshes are streamed asynchronously (dedicated extraction worker, 55/80 m hysteresis); collider registration does not wait for the mesh. `topologyToCaveDefinition` / `CaveVolume` remain catalog/streaming leftovers until plan world-terrain-008 Milestone B5. An isolated `?caveHeightfieldTest` harness (plan world-terrain-018) compares a 2.5D floor+ceiling+boundary-wall representation against the same `CaveTopology` in production SDF, without booting `WorldBundle`; both variants are built on the production analytic terrain sampler (`sampleHeightAt` + `defaultTerrainConfig`, no `ChunkManager`) and resolve Walk-mode ground through the production Cave V2 contract (`pickInterval` → `applyCaveGroundHysteresis`); it does not change production cave behaviour.
