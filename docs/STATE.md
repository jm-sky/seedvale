...
- Prey spawners (cave / thicket) with respawn; placement rejects roads, coast/beach, and other spawn points; village avoidance uses each settlement’s real footprint radius. A `wolfDen` spawner (plan 093 Etap E) reuses the same shape/prop for a one-time, non-respawning wolf pack (quest target, not a nuisance spawner). Spawn-point population lifecycle (plan 125): each managed `PreySpawner` (cave/thicket, not `wolfDen`) carries a stable `id` (`settlementId:type`), a `SpawnPointState` (`active`/`depleted`/`disabled`/`recovering`) and `deathsThisCycle` — `AnimalAgent.spawnPointId` (set only for spawner-created animals) routes each death through the existing `onDeath` hook to the owning spawner, no double-counting; `>50%` of `maxPreyCount` deaths moves it to `depleted` (no more respawn); `[E] Zniszcz` there is a 5 s busy channel (plan 137, 4 branches spent on complete) that moves it to `disabled`, lights a `PlacedFires` `pit` with those 4 branches as ~5 min fuel, tints the spawner prop near-black (mesh stays) and applies `ChunkManager.scorchTerrain()` (r≈7 charcoal vertex tint + `roadTint` grass fade); after `RECOVERY_DAYS` (21) with ≥2 live same-kind animals nearby it returns to `active` with counters reset — checked at most once per in-game day, not per frame. Lifecycle state is not persisted (`docs/plans/LOOSE-ENDS.md`).
