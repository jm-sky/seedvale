...
- Tree/bush leaf materials share a cheap vertex wind (`world/foliageWind.ts`); GLTF `BLEND` foliage is hardened to opaque `alphaTest` cutouts so canopies write depth (issue 022). Post-processing ends with a subtle film grade + Bayer dither, merged into the output pass (`render/gradedOutputPass.ts`, review 005 A3.1).
- Ocean (`createOcean` / `waterMaterial.ts` / `waterMirror.ts`) shares the lake shader family (`transparent` + `depthWrite: false`, world-space waves, depth fade from `floorHeights`). Terrain mesh under water follows `floorHeights` (bathtub), while clamped `heights` stay the walk/mask lid. One 256² planar mirror for lakes and ocean (`postProcessing.waterReflections`, Vue Pauza → Świat → Grafika). Chunk water draws ocean cells with a `vCover` shore fade; the singleton fills open sea beyond `loadRadius`. Lakes never use ocean styling from chunk area (plan 098 / W8). Water domain SoT: [WATER.md](./WATER.md).
...
- Per-chunk vegetation and natural environment elements. Living trees, bushes, desert cacti (`cactus_a`/`cactus_b`), swamp reeds (`reed_a` cattail), and GLB rocks/clusters/logs are `InstancedMesh` buckets per (species, primitive, chunk) (`src/render/instancedProps.ts`, plan 087); distant chunks thin the draw range via `InstancedMesh.count` (same prefix-LOD as grass). Procedural `createCactus`/`createReed` remain load fallbacks. Stage meshes (limbed/felled/stump) and procedural landmarks stay individual `Object3D`s. Settlement trees/bushes are not instanced.
...
- Settlement generation with families, houses, roads/paths and environment-aware siting. Open-ocean grid cells are skipped (no wet fallback); home widens the site search if spawn is wet (issue 029).
...
- Village gardens scale (plan 077): ~1 garden unit per 3 houses packed into S/M/L beds; garden clearings keep trees out; `landmarks.gardens[]` with primary `garden` for farmers. Garden centers stay outside the plaza disk (`plazaCoreRadius + gardenPlotRadius`, plan 095) in home and non-home villages. Garden mesh is `crops.glb` (tiled for M/L, `CROPS_FIT_MAX` 4.8); `createGarden` is fallback. Mesh sits on `clearings.gardens` (house-level flatten + dirt tint hugging the beds; plan 100). Settlements with `foodSourceType === 'field'` also place `farm.glb` at the `field` landmark (`createWheatField` fallback) (plan 099).
...
- Well drinks use a shared per-settlement `InteractionQueue` (plan 079): FIFO standing slots south of the well, one serving agent at a time; home drinks skip the queue. Same queue type is reusable for future garden/stall points. Village well mesh is `well.glb` (`createWell` fallback); drink-queue rim still `settlement:well` (plan 101). Stockpile visual is `wood_pile.glb` (`createStockpile` fallback); Fantasy RTS `logs.glb` is parked.
