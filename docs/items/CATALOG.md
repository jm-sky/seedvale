...
**Last updated:** 2026-09-18 (dagger/hatchet/masterwork_hunting_bow compact weapon niches, plan items-player-047; previously 2026-09-16 armor quality `poor` + instance-aware merchant pricing, plan items-player-040; previously 2026-09-15 ring_gold/ring_turquoise/ring_amethyst loot jewelry added, M88–M90; previously plan quests-progression-035 — story/other item categories)
...
| Holdable (Weź) | `ITEM_CATALOG[kind].holdable` — the single source of truth; `HeldTool.ts`'s `isToolKind` reads the derived `HOLDABLE_KINDS` (plan 184). 25 kinds: knife, firestarter, shovel, axe, wooden_torch, pickaxe, long_sword, spear, short_sword, pitchfork, sickle, shears, damascus_knife, damascus_short_sword, damascus_long_sword, obsidian_sword, battle_axe, masterwork_sword, dagger, hatchet, fishing_rod, short_bow, hunting_bow, long_bow, masterwork_hunting_bow (plan items-player-047) |
| Tool capabilities (plan 184) | `ITEM_CATALOG[kind].capabilities` (`ItemCapability`) — the single source of truth for every "can this item do X?" gate, replacing `kind === 'shovel'` / `isChopTool()` / `isHarvestKnife()` checks. `wood_chopping` (axe, battle_axe) · `meat_harvesting` + `branch_trimming` (knife, damascus_knife, dagger) · `soil_digging` (shovel — dig/level soil, bury a corpse, well `pit` stage) · `rock_mining` (pickaxe — rock dig/level + ore deposits) · `fire_starting` (firestarter) · `fishing` (fishing_rod) · `textile_repair` (`sewing_kit`, carried non-holdable utility — tent/bedroll repair, plan items-player-019) · `prying` (pickaxe, axe, battle_axe, pitchfork — forced entry and other leverage, plan items-player-026) · `shearing` (`shears` — sheep wool, plan fauna-004). `hatchet` (plan items-player-047) deliberately declares no capabilities — a compact personal weapon, not a working tool. Query with `hasItemCapability(kind, cap)` for the hand, `Inventory.hasCapability(cap)` for the bag, `Inventory.findWithCapability(cap)` (best-first) when a caller must auto-equip. `melee`/`ranged`/`defense`/`consumable`/`food.bait` stay their own configs — they already answer capability questions. |
| Weapon maintenance (plan 161) | `items/itemInstances.ts`'s `WEAPON_MAINTENANCE_KINDS` (15 kinds: knife, short_sword, long_sword, spear, axe, pitchfork, sickle + the six plan-160 variants + `dagger`/`hatchet`, plan items-player-047) are `ItemInstance`-backed with `durability`/`sharpness` (`[0,1]`, new = 1/1); `shovel`/`pickaxe` are explicitly excluded. `HeldTool.heldInstanceId()` tracks which concrete instance is in hand. `items/weaponMaintenance.ts` — `getSharpnessDamageModifier()` (100%→100%…0%→55%) feeds melee damage before the critical roll; sharpness/durability wear applies once per resolved hit via `Inventory.updateInstance()`. `whetstone` (stackable) + `sharpenWeapon()` restore sharpness only, never durability; no repair/broken lifecycle in v1. |
| Ranged combat (plan 162) | `ITEM_CATALOG[kind].ranged` (`RangedConfig`) on `short_bow`/`hunting_bow`/`long_bow`/`masterwork_hunting_bow` (elite quest-only variant, plan items-player-047) — `player/playerRanged.ts` runs the draw→release→recovery lifecycle (same shape as melee); `combat/projectile.ts` is the lightweight swept-segment flight/collision model (no visual arrow mesh in v1); `combat/rangedAttack.ts` turns bow accuracy + `archery` skill into aim deviation, not a separate hit-roll. Ammo (`arrow`/`broadhead_arrow`/`war_arrow`) is ordinary stackable count, 1 consumed per shot, no per-arrow instance/recovery. `masterwork_hunting_bow` is never `MERCHANT_STOCK`/`PREMIUM_MERCHANT_KINDS` — quest/exceptional-reward only, no bow durability (matches the other bows). |
...
| Wearable multi-part armor (plan items-player-030 / items-player-040) | `ITEM_CATALOG[kind].armor` (`ArmorConfig`) on `leather_armor`/`chainmail` and pauldron kinds — six equipment slots (`head`/`body`/`arms`/`hands`/`legs`/`feet`); `items/equipment.ts` tracks worn *instance ids*; `quality` (`poor`/`common`/`good`/`masterwork`) on `ArmorItemInstance` adjusts protection, effective weight and restriction multipliers via one resolver (`poor` is a craftsmanship tier, not wear). `resolveEquipmentModifiers()` folds all live-valid pieces (remaining-damage product + multiplier product). Passive armor applies after active held-item block; never to starvation/dehydration. Weight uses effective instance mass for encumbrance. Merchant catalog prices stay the `common` baseline; concrete buy/sell value uses `armorQualityValueMultiplier`. New piece kinds remain asset-gated. |
...
| Melee vs animals | `ITEM_CATALOG[kind].melee` (plan 123, `itemCatalog.ts`) — single source of truth for damage/range/arcDot/windUp/hitWindow/recovery/staminaCost; `player/playerMelee.ts` runs the windUp→hitWindow→recovery lifecycle + range/facing-arc hit test. `faunaCombat.ts`'s `isMeleeTool()` just reads this. Damage: obsidian_sword 46, damascus_long_sword 40, masterwork_sword 34, long_sword/battle_axe 28, damascus_short_sword 24, spear/axe 20, hatchet/short_sword 18, damascus_knife 16, dagger 14, pitchfork 14, knife/sickle 12, shovel/shears 8 (dagger/hatchet — plan items-player-047 compact weapon niches) |
...
| Inventory category | `ITEM_DEFS.categories` — `resource` / `tool` / `utility` / `food` / `weapon` / `armor` / `knowledge` / `story` / `other` (multi-category, e.g. axe = tool + weapon); wearable armor (`leather_armor`, `chainmail`, …) is `armor`, not `weapon`; hunger consumables are `food`, waterskins stay `utility`. `knowledge` (plan items-player-016) covers `map_near`/`map_far` (→ `LocationKnowledge`) and the 18 skill books (→ `PlayerSkills`) — a shared item category, not a shared gameplay system. `story` (plan quests-progression-035) is presentation-only for quest/history items (`treasure_map_dark_forest`, `signet_ring`, `bandit_ledger`, `marked_valuable`, `expedition_journal`, `encoded_chronicle`, `chronicle_search_evidence`); it does not itself change sell/drop/ownership rules. `other` is an explicit leftover bucket, never a runtime fallback for a missing `categories` list |
...
| shears | nożyce do wełny | yes | 8 | none (Kupiec + shepherd loadout) | procedural | plan fauna-004; `shearing` capability gates sheep wool harvest |
...
| tent | namiot | — | — | none (Kupiec) | procedural | place / rest / pack (plan 090); instance-backed `TentItemInstance` with 0..100 condition, same id as the placed tent (plan items-player-019); roads allowed (issue 035) |
...
| ruby | rubin | — | — | none (authored dark-forest chest) | procedural | plan quests-progression-009; legacy unsized gem; keep for existing saves/authored loot; `physical.resilient` |
| ruby_small / ruby_medium / ruby_large | mały rubin / rubin / duży rubin | — | — | systemic treasure | procedural | plan items-player-026; sized ruby treasure; `physical.resilient`; trade value 30/70/150 |
| diamond_small / diamond_medium / diamond_large | mały diament / diament / duży diament | — | — | systemic treasure | procedural | plan items-player-026; sized diamond treasure; `physical.resilient`; trade value 80/180/400 |
...
| sewing_kit | zestaw do szycia | — | — | none (Kupiec, 18 coin) | procedural | plan items-player-019; reusable non-holdable utility; `textile_repair` gates tent/bedroll repair (never `kind === 'sewing_kit'`) |
...
| dagger | sztylet | yes | 14 | none (Kupiec) | procedural (needed) | plan items-player-047; niche between knife and damascus_knife; harvests corpses like knife |
| hatchet | toporek | yes | 18 | none (Kupiec) | procedural (needed) | plan items-player-047; compact one-handed axe; no `wood_chopping` (not a working tool like axe/battle_axe) |
| masterwork_hunting_bow | mistrzowski łuk myśliwski | yes | ranged 24 | quest/exceptional only — never `MERCHANT_STOCK`/`PREMIUM_MERCHANT_KINDS` | procedural (needed) | plan items-player-047; fastest draw + highest accuracy of the three bows, less raw damage than long_bow; trade value 220 (barter/inventory only) |
...
2. ~~Item durability / HP~~ — implemented for the 15-kind weapon-maintenance
