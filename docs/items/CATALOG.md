...
**Last updated:** 2026-09-12 (plan items-player-029 — wearable body armor)
...
| Wearable body armor (plan items-player-029) | `ITEM_CATALOG[kind].armor` (`ArmorConfig`) on `leather_armor`/`chainmail` — `items/equipment.ts`'s `EquipmentState` tracks which owned item is worn in the (only) `body` slot; `resolveEquipmentModifiers()` is the single pure derivation point damage/melee/movement code reads. Passive `damageReduction` applies to player combat/physical damage *after* active held-item block (`combat/defenseResolver.ts`) — a distinct mechanic, never folded into `DefenseConfig` — and never to starvation/dehydration. `staminaCostMultiplier`/`meleeRecoveryMultiplier` scale melee effort/cadence at use time without mutating `MeleeConfig`; `movementSpeedMultiplier` composes with (not instead of) carry-weight encumbrance; `sprintStaminaMultiplier` scales the sprint Stamina drain. Player-only in V1 — no durability, no head/hands/legs/offHand slots, no NPC equipment. A third, lighter "padded gambeson" tier is deferred pending a confirmed asset. |
