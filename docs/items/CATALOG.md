...
| Wearable multi-part armor (plan items-player-030) | `ITEM_CATALOG[kind].armor` (`ArmorConfig`) on `leather_armor`/`chainmail` — six equipment slots (`head`/`body`/`arms`/`hands`/`legs`/`feet`); `items/equipment.ts` tracks worn *instance ids*; `quality` (`common`/`good`/`masterwork`) on `ArmorItemInstance` adjusts protection, effective weight and restriction multipliers via one resolver. `resolveEquipmentModifiers()` folds all live-valid pieces (remaining-damage product + multiplier product). Passive armor applies after active held-item block; never to starvation/dehydration. Weight uses effective instance mass for encumbrance. New piece kinds remain asset-gated. |
