...
| S25 | NPC campfire spoken conversation | Opposite-gender Q/A exchange when a Social Place `conversation` begins (plan npc-045); legacy friendly-talk murmur still the fallback for same-gender / non-campfire | `wired` | Structured pairs via `ai/campfireTalk.ts` + cues from `advanceSocialPairing` → `NpcAgent.beginConversation` (`public/sounds/voices/campfire_*.mp3`). Empty `NPC_FRIENDLY_TALK_SOUND_URLS` remains the silent/future murmur fallback when no authored exchange exists. |
