...
| S07 | Rain | Weather = `rain` or `storm` (plan 040 Etap 1 / world-026, `audio/weatherSounds.ts`) | `wired` | `ambient-rain-loop-01`; storm uses the same loop at higher gain |
...
| S28 | Thunder (several one-shot variants) | Storm lightning events (plan world-026, `audio/weatherSounds.ts`) | `needed` | Plumbing wired to `/sounds/weather-thunder-01.ogg`…`03.ogg`; files are not in the repo — playback warns and stays silent until acquired. Storm rain/wind reuse existing rain + wind loops |
