...
| M53 | Storage chest | Player-placed container (`items.chest`); procedural box+lid fallback (`world/containerProp.ts`) wired now. Plan 169 reuses this same procedural chest for the house-interior chest furniture (`COTTAGE_4X4_A`) — no second chest asset | `wired` (procedural) | [plan 164](../plans/2026-08-19--164--player-storage-and-container-system.md), [plan 169](../plans/2026-08-19--169--house-interior-furniture-and-bed-anchors.md) |
...
| M62 | House interior bed | `COTTAGE_4X4_A` sleep furniture — provides plan 168's `bed` `LodgingOption` via `HouseAssembly`'s `'sleep'` interaction point. Quaternius Furniture Pack `Bed.fbx`, converted `FBX2glTF` → `gltfpack -cc`, resolves through `ConstructionCatalog` like any other house part | `wired` | plan 169 (`settlement/furniture/bed.glb`) |
| M63 | House interior table | `COTTAGE_4X4_A` static furniture; also the mount anchor origin for M64 | `wired` | plan 169 (`settlement/furniture/table.glb`) |
...
| Settlement (active) | MegaKit assembled homes (`HouseBuilder`), huts/towerhouse (catalog fallback + Asset Browser), wall stubs, dock, crate/barrel, garden/crops/storage, wood pile, hay, wheat field (`farm.glb`), well GLB, plaza cobble clutter (MD+), interior furniture bed/table/lamp (`COTTAGE_4X4_A`, plan 169) |
