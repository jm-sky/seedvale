...
| M74 | Lily pad (shallow-water surface vegetation) | New `VegetationKind: 'lily'`, `LILY_SPECS` — dedicated `lilyPatches()` pass in `chunkVegetation.ts` places bounded, patch-based clusters on shallow inland lake water only (never ocean, never a flowing river channel). Flat/wide asset, fit via `preparePropFitMax` (not the usual height fit); procedural flat-disc fallback `createLilyPad`. Reed cluster variants (denser reeds) and shallow coastal seaweed from the same plan were **not** added — no such assets exist and none were authored this pass | `wired` | plan world-terrain-010 (`parked/Lilypad-01.glb`, formerly parked) |
...
| Nature (active) | trees/bushes/pines, fern undergrowth, cactus/reed, lily pad (shallow water, see M74), rock/log, ore piles, cemetery / gravestones |
