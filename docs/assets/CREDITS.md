...
| `public/models/characters/ubc/male_peasant.glb`, `male_ranger.glb` (player, `?player=`) | Universal Base Characters + Modular Character Outfits - Fantasy / Quaternius | [quaternius.com](https://quaternius.com/packs/universalbasecharacters.html) | CC0 1.0 | `Superhero_Male_FullBody` (head slice) + `Hair_SimpleParted` + `Male_Peasant` / `Male_Ranger`; compose script `scripts/assets/compose_ubc_player.py` (plan items-player-033) |
| `public/models/characters/ubc/ual1_player.glb` (player clips, `?player=`) | Universal Animation Library / Quaternius | [quaternius.com](https://quaternius.com/packs.html) | CC0 1.0 | `UAL1_Standard.glb` in-place subset `Idle_Loop` / `Walk_Loop` / `Sprint_Loop` / `Sword_Attack` |
