...
| `public/models/_main_character/optimized/base_male.glb` | Universal Base Characters / Quaternius | [quaternius.com](https://quaternius.com/) | CC0 1.0 | `Superhero_Male_FullBody` — player character (plan 172); converted from `models/Superhero_Male_FullBody.gltf` (`gltf-transform`: textures 4096→1024px + WebP only, **no meshopt** — see note below) — 11 MB → 831 KB. Source `.gltf` had two broken texture URIs (`_png.png` exporter artifact) fixed in place before conversion |
| `public/models/_main_character/optimized/outfit_male_peasant.glb` | Modular Character Outfits / Quaternius | [quaternius.com](https://quaternius.com/) | CC0 1.0 | `Male_Peasant` — player outfit (plan 172); same rig as `base_male.glb` (verified identical 65-joint names/order) so the outfit's `SkinnedMesh`es rebind onto the base skeleton at load time instead of keeping a second skeleton; converted from `outfits/Male_Peasant.gltf` same way — 6.9 MB → 882 KB |
| `public/models/_main_character/animations/player_locomotion.glb` | Universal Animation Library[Standard] / Quaternius | [quaternius.com](https://quaternius.com/) | CC0 1.0 | `Idle_Loop`/`Walk_Loop`/`Sprint_Loop`/`Sword_Attack` on the same 65-joint rig as `base_male.glb` (base character pack itself ships no animations) — non-root-motion variant (`UAL1_Standard.glb`, not `_RM`), trimmed to those 4 clips + meshopt-compressed — 7.6 MB → 226 KB. License: [quaternius-universal-animation-library-license.txt](./quaternius-universal-animation-library-license.txt) |
| `public/models/_main_character/{models,outfits}/*` (female base, Ranger outfits, raw male source) | Universal Base Characters + Modular Character Outfits / Quaternius | [quaternius.com](https://quaternius.com/) | CC0 1.0 | Unconverted source files (4096px textures) — **parked**, not wired; female-character follow-up to plan 172 |
...
Universal Animation Library: [quaternius-universal-animation-library-license.txt](./quaternius-universal-animation-library-license.txt).

**Uwaga (Universal Base Characters / Modular Character Outfits):** jak `tree_c`/`bush_*` — brak osobnego pliku licencji w zestawie checked-in do repo (commit `5bc3aee`); jak wszystkie paczki Quaternius, licencja to CC0 1.0.

**Uwaga (base_male.glb / outfit_male_peasant.glb — NIE stosować `gltf-transform meshopt` na tych plikach):** pierwsza konwersja przepuściła oba pliki dodatkowo przez `gltf-transform meshopt --level medium` (jak reszta assetów w tym pliku) i to **wizualnie zepsuło strój** — meshopt's per-mesh quantization przekształciło/rozjechało `inverseBindMatrices` niezależnie dla każdego z 3–5 skinów w jednym pliku (zweryfikowane: `boneInverses[0]` scale różnił się między meshami tego samego stroju — 0.899 vs 0.373 vs bazowe 0.062 — mimo identycznej listy kości), więc strój po zbindowaniu na szkielecie bazowej postaci zapadał się w jeden mały punkt zamiast pokrywać ciało. Bieżące pliki są **tylko** po `textureCompress` (1024px WebP) — bez meshopt geometrii/skinningu — stąd większy rozmiar niż typowy `gltfpack -cc` wpis w tej tabeli. Jeśli ktoś w przyszłości spróbuje domeshoptować te pliki (albo przekonwertować female base/outfit tym samym pipeline'em), musi zweryfikować `skeleton.boneInverses` po konwersji (patrz `docs/plans/2026-08-20--172--modular-quaternius-player-character.md`), nie tylko rozmiar/liczbę kości.
