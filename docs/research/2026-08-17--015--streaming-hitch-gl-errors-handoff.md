...
> **Correction (2026-08-17, [research 017](2026-08-17--017--threejs-rendering-audit.md) §3.1):** the paragraph below is **wrong about the live pipeline**. Seedvale never renders the scene to the canvas — the beauty pass goes into `EffectComposer`'s render target too, so `WebGLPrograms.getParameters()` gives *both* the mirror and the main scene pass `NoToneMapping` + `workingColorSpace`. They share one program variant. The mirror dominates the traces because it runs **first** in the frame (`gameLoop.ts:995` before `:1004`) and therefore absorbs each newly streamed material's one-time link wait — not because it needs a second variant. This retires §5's angle-3 as written.
