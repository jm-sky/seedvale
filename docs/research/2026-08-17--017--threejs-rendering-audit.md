...
| F5 | **AO diagnostic code in `createPostProcessing.ts`** | Left in place at the user's explicit direction. Note that it is an **active deviation from intended behaviour**: `__aoDiag` is exported to `globalThis`, N8AO is pinned on, plan 113's heavy-frame auto-suppress does not actually apply (`applyFrameBudget` only logs), and it prints `console.info` per flip. Belongs to a separate in-flight ~20 Hz tremble investigation (research 015 §7). Any AO benchmark run before this is reverted measures a pinned-on AO pass. |
...
- **The AO diagnostic code is still active** (F5), so AO behaviour differs from plan 113's intent.
- Verification split:
  - **Implemented:** I1–I5.
