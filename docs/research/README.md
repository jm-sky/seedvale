...
| [2026-08-16--013--compileasync-prewarming-plan.md](2026-08-16--013--compileasync-prewarming-plan.md) | `renderer.compileAsync()` A/B experiment plan (diagnostic only, no code changes): exact integration point, mirror-variant mechanics, risks (KHR extension support, lead-time window) — flags that `checkShaderErrors=false` (commit aaeee34) is already committed to `main`, contradicting the task's premise | `planned` |
