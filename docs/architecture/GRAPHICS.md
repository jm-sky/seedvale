...
| G9 | Droga = tint korytarza na meshu terenu (nie osobny mesh). Miękki brzeg + ziarno dirtu; trawa **soft-fade** w korytarzu, nie hard bald cut. Extra gęstość łąki = **filler LOD** (coverage 0..1, quality knob), nie globalny bump `grass.density`. | `chunkHeightmap` / `biomeColors` / `grass` / `chunkManager`, issue [023](../issues/2026-08-12--023--road-grass-ground-cover.md), plan [world-terrain-005](../plans/world-terrain-005-distance-based-terrain-detail-lod.md) |
| G18 | Road near-field surface detail (koleiny/grudki) = **real vertex displacement baked into the existing corridor height blend** (`roadCandidate`), reusing the pothole mechanism's noise handle/apron sampling — not a shader-only distance-faded displacement, and not a new `RoadMesh`. Rationale: at this scale (few cm) the bumps are imperceptible past typical camera distance anyway (no LOD mechanism needed for the "far stays cheap" half of the contract), and reusing the pothole path means **one** source of road-height truth instead of a second render-only one. Independently toggleable (`surfaceDetailEnabled`) from potholes/edge-wobble. | `chunkHeightmap.ts`'s `roadCandidate`, plan [world-terrain-005](../plans/world-terrain-005-distance-based-terrain-detail-lod.md) |
...
| G17 | Chunk mesh pipeline (stan kodu): `chunkWorkerPool` / `chunkHeightmap.worker.ts` generuje poza main threadem zarówno dane kafla, jak i CPU-heavy dane geometrii chunku. `chunkMeshData.ts` produkuje data-only `ChunkMeshData` (Y, normals, vertex colors, `aBareGround`) w istniejącym worker poolu; main thread pozostaje odpowiedzialny za finalne utworzenie `THREE.PlaneGeometry` / `Mesh` i scene attach. Mesh jest osobnym `mesh` jobem uruchamianym po zastosowaniu runtime terrain modifications do tile, dzięki czemu zachowana jest istniejąca semantyka `dig` / `scorch` / `prepare`. `ChunkMeshData` jest cache'owany w bounded runtime LRU (`chunkMeshCache.ts`, domyślnie 64 MB); globalny `modificationsEpoch` bezpiecznie invaliduje cache po zmianach terenu. Stale worker results są odrzucane przez request sequence / chunk identity. Historyczny benchmark przed tą zmianą wykazywał 51× `chunk mesh`, avg ~45.5 ms, max ~92.6 ms; wartości te są baseline sprzed implementacji, nie opisem obecnego kosztu. | `src/terrain/chunkMeshData.ts`, `src/terrain/chunkMeshCache.ts`, `src/terrain/chunkWorkerPool.ts`, `src/terrain/chunkHeightmap.worker.ts`, `src/terrain/buildChunkGeometry.ts`, `src/terrain/chunkManager.ts`, plan `world-terrain-004` |
...
- Grass: `grassFillerLodFraction` (`distanceLod.ts`) now takes a `radius` argument instead of a hardcoded near-only cutoff (`dist <= 1`) — `ChunkManager`'s new `grassFillerCoverage` quality knob (0..1, `WorldConfig.quality`, live/no-rebuild) scales how far across `effectiveGrassRadius` the cheap filler bucket reaches. Reuses the existing filler `InstancedMesh`es (already generated for every grass chunk regardless of coverage) — only the draw fraction changes, same live-uniform pattern as `setLodScale`. See G9.
- Road: `RoadNetworkParams` gains `surfaceDetailEnabled`/`rutDepth`/`rutOffsetFraction`/`rutWidthFraction`/`microBumpStrength`/`microBumpScale`, applied in `roadCandidate()` right next to the existing pothole term — two symmetric wheel-rut grooves (one Gaussian centered on unsigned lateral distance, since ruts are symmetric) plus continuous (non-sparse) fine bump/dip noise, both scaled by the segment's own `falloff`/`heightStrength` so paths stay nearly flat like potholes already do. See G18 for why this is baked geometry, not a shader effect.
- No new `RoadMesh`, no new worker/geometry attribute, no chunk-mesh-resolution increase — both features extend an existing generation/LOD mechanism. Debug GUI: "Grass filler coverage" slider (Post-processing folder) and new sliders in "Roads" folder.
...
### 2026-09-02 — Chunk mesh worker migration + geometry cache (world-terrain-004) 🔧

- CPU-heavy chunk mesh-data generation zostało przeniesione do istniejącego `ChunkWorkerPool` jako osobny `mesh` job; nie powstał drugi worker system.
- `chunkMeshData.ts` generuje data-only mesh attributes poza main threadem. Main thread zachowuje ownership obiektów Three.js i scene attach.
- Runtime terrain modifications są nadal aplikowane przez istniejącą logikę przed mesh requestem, dzięki czemu `dig` / `scorch` / `prepare` zachowują dotychczasową semantykę.
- Dodano bounded runtime `ChunkMeshData` cache (`chunkMeshCache.ts`, domyślnie 64 MB) z bezpieczną invalidacją przez `modificationsEpoch`.
- Historyczne `51× / avg ~45.5 ms / max ~92.6 ms` opisują benchmark sprzed migracji i nie powinny być traktowane jako aktualny koszt pipeline'u.
