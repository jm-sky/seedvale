...
2. **The `WorldBundle`-rebuild invariant is stated independently in three files.** `CLAUDE.md` ("Important architecture"), `docs/STATE.md` ("Runtime architecture"), and `docs/architecture/ARCHITECTURE.md` ("World lifecycle" + "Rebuild / lifetime invariants") each restate, in different words, "don't capture a replaceable bundle field in a stale closure." If the invariant changes, three places need edits, and drift is not hypothetical — see #3.
3. **Two "source of truth" docs actively disagree on current facts, not just omit them.** `CLAUDE.md`'s "Important entry points" file list is already stale relative to `STATE.md`'s — `STATE.md`'s equivalent list has grown to include `houseBuilder.ts`, `props.ts`, `constructionCatalog.ts`, `economy/`, `campfireCooking.ts`, `PlayerNeeds.ts`, `WaterSource.ts`, `dayNight.ts`, `treeLifecycle.ts`; `CLAUDE.md`'s list is the older, shorter subset. Worse, `docs/architecture/ARCHITECTURE.md` states "The current save schema is version **11**" (its own "Last verified" date: 2026-08-14), while `docs/STATE.md` — written one day later — states "Canonical save schema is **v13**." `src/persistence/saveData.ts` confirms `STATE.md` is correct: 13 versioned schema steps exist in code, with explicit v10→v11→v12→v13 migrations. This isn't a stale list an agent can shrug off by reading `STATE.md` instead of `CLAUDE.md` — an agent that opens `ARCHITECTURE.md` first for a persistence-related task gets a materially wrong, two-versions-behind fact stated as current.
...
**Current domains exist, but as three inconsistent taxonomies, not one.** `docs/plans/archive/README.md`'s own two sections don't agree with each other: its "Done" grouping merges NPC and settlement work into one bucket (`Osady / NPC`), while its "Verification needed" section splits the same territory into `Osady / wioski` and `NPC` as separate buckets, and further splits out `Las / narzędzia / zbieractwo` (items/tools/gathering) as its own category. "Done" also has a `Fundament` bucket that conflates terrain, rendering and post-processing, and an `Itemy / świat` bucket that conflates items, quests, minimap and lighting. `STATE.md`'s 7 section headers (World/terrain, Settlements/NPCs, Fauna, Items/player, Quests/progression, Persistence, UI/input) are cleaner and closer to the "Done" grouping than to "Verification needed" — but match neither exactly. The mechanism recommended below is unchanged, but the domain *vocabulary* needs to be picked once, explicitly (`STATE.md`'s 7 headers are the best existing candidate), not assumed to already be settled from the archive.
...
3. Fix `ARCHITECTURE.md`'s stale save-schema claim (§B.3): "version 11" → "version 13," matching `STATE.md` and `src/persistence/saveData.ts`. A one-line fix, but it's a live factual error in a "source of truth" doc, not just a structural duplication risk.
4. Resolve `docs/roadmap/`'s status (§B.4): either fold its conclusions into `docs/ROADMAP.md` and delete the directory, or add it to `docs/README.md`'s nav table with an explicit note on its relationship to `ROADMAP.md`.
5. Add a minimal CI workflow (`tsc`/lint/build/test on PR) — highest-leverage single change given concurrent multi-agent editing.
6. Add an explicit git-workflow section to `CLAUDE.md` (commit → `pull --rebase` → push is part of "done"; never force-push/`reset --hard`; merge concurrent changes rather than blocking on them) per the audit brief — currently unstated.

**🟡 Next**

7. Pick one canonical domain vocabulary for plans (§D) — `STATE.md`'s 7 headers are the best existing candidate — and reconcile `docs/plans/archive/README.md`'s two internally-inconsistent groupings against it before adding `domain:`/`tags:` frontmatter to new plans going forward (no retrofit of the 147 existing files).
8. Add a `validate:plans` script (index-completeness + ID/date-format check across `plans/`/`reviews/`/`issues/`/`research/`) and wire `scripts/audit-megakit.mjs` into `npm run`.
9. Fix or delete `docs/prompts/000` (stale naming convention, §B.8).
10. Give Cursor a minimal repo-scoped rules file — even a short one pointing at `CLAUDE.md`'s core rules, if Cursor's rule format supports an include, otherwise a trimmed copy.
