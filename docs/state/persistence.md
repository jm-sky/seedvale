...
| NPCs | Authoritative state (health/stamina/vigor/needs/injury/helper-assignment/active-plan/post-death/grave-visit history/personal inventory/transport cargo) and both relationship stores persist. Identity/physical profile is deterministic. Decision/execution runtime state and transient `carried` work inventory never persist (loadout belongings become corpse loot at death; transport cargo is a distinct, persistent third inventory — see [npc.md](./npc.md)). Active/non-terminal `TransportOrder` commitments persist as sparse-optional `SaveData.transportOrders` (plan settlements-npcs-019), same idiom as `npcStates`/`households`. |
