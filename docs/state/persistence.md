...
Beyond that, core terrain/hydrology generation is still deterministic reconstruction; its in-session caches (chunk mesh data, river tiles, settlement-plan memoization) are ordinary evictable performance caches over pure functions, not versioned across sessions.
