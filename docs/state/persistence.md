...
| Quests/progression | Persisted authoritative (progress including `resolvedOutcomeId` after a complete/failed outcome, optional `journal` heard-line stamps, player↔NPC relations). Quote text is projected from live defs, not stored. Global quest EXP is not persisted. |
