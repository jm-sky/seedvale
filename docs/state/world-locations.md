...
→ player/world discovery: a discovered cell or location is recorded
  (Fog of War cells stay independent of location knowledge;
   home settlement is confirmed at boot; caves/settlements confirm on
   throttled physical proximity — VillagePlan.boundary for villages)
...
- **Deferred world knowledge:** `WorldKnowledgeResearch` (`src/world/locations/worldKnowledgeResearch.ts`) owns worker-backed static-world lookup (nearest/nearby lightweight landmarks). The main thread posts a cheap seed/config/query snapshot; the `worldKnowledge` worker reconstructs per-chunk corridors and resolves landmarks (plan world-030). `QuestManager` persists quest knowledge slots; the „Opowiedz mi coś o okolicy” topic (home guard and eligible local hunters) is owner-keyed per asking NPC (`SaveData.map.guardLocalKnowledge`, `npcId → wait/selected-ids` — a legacy single-record pre-npc-050 save migrates onto the stable home-guard id at restore time) and, once research resolves and the authored delay elapses, transfers the ready result into that NPC's own authoritative `playerFollowUp` (plan npc-050) rather than revealing it directly — see [npc.md](./npc.md)'s NPC-initiated Player follow-ups section for the delivery seam. Neither path stores worker handles. Discovery still goes through `LocationKnowledge`.
...
- **Persistence:** discovery/knowledge/target state persists as part of `SaveData.map` (optional `guardLocalKnowledge`, an `npcId`-keyed wait/selection map, plan npc-050); location geometry itself is never persisted, always re-derived from `(world seed, location id)`. See [persistence.md](./persistence.md).
