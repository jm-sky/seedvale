...
`AnimalDef` (one entry per `AnimalKind` — predator/prey/livestock species) is the single per-species data table: role, sociability, speeds, detect/flee/notice/panic ranges, optional `fearBaseline` for sudden world stimuli such as thunder, and a set of optional capability blocks that follow a consistent convention: **the presence of the field is the capability**, not a separate boolean flag. `mount` (ridable), `production` (egg/milk), `scavenging` (optional per-phase rotting-corpse/bones food value — wolf rotting+bones, bear rotting-only, fox fresh-only), `diet` (grass/item herbivore, meat, or bear's mixed meat+plant diet), `water` (swim capability), `roaming` (species wander band), `trips` (periodic long-range water trips — deer/stag, and now bear, plan fauna-019 §7). A dog "eats meat but doesn't hunt" is expressed purely by its role not being `predator`, gating the carcass-seeking food branch — not a dog-specific flag. Bear's `diet` is likewise inert for hunger search (`findFoodTarget()` still takes the predator carcass branch) and exists for dropped-food / trap-bait attraction (plan fauna-023).
...
**Roaming:** each species has a home-relative wander band (two named size tiers, plus a flat fallback for species without an override); livestock gets its own fixed band instead. A larger, separate hard cap (`ROAM_RADIUS`, also home-relative) bounds ordinary local wander/forage search regardless of species tier, but no longer a committed trip: `AnimalAgent.clampBounds()` skips its clamp entirely while a trip (`AnimalTrip`) is active, so a trip's own destination search radius is the only bound on how far it may travel (plan fauna-019 §6) — deer/stag and bear each get a periodic long-range water "trip" that deliberately exceeds their own roaming band, and a cave-bound resident's route to/from its entrance is not expressible as a flat radius around `home` at all.

**Habitat/spawning:** wild-fauna "ring" spawns are placed at a settlement-relative offset per habitat profile (open/meadow/forest/water/edge), gated by river-channel clearance and habitat checks; `rockDen`/thicket/wolfDen habitat spawners (`rockDen` is the decorative `createCaveMouth()` prop, renamed from `cave` — plan fauna-019 §8 — to stop colliding with real walk-in caves; `spawnerId()` still derives the on-disk `cave` id segment for save compatibility) are placed further out and tracked as the separate spawner state machine above. Closed-settlement predator pressure (plan settlements-010) is a generation-time configured-capacity fill (`closedPredatorPressure.ts`): it reads nearby predator `maxPreyCount` (including `wolfDen` as natural pressure), adds only the local deficit versus an absolute closed floor, never live counts, never mutates `wolfDen`, and places extras through the same habitat validator with a dedicated RNG and stable `${settlementId}:closed-pressure:${slot}` ids. The thicket (`zagajnik`) marker is four cloned nature GLBs (`tree_a`/`tree_b`/`tree_c`/`birch_1`, `THICKET_TREE_SPECS`) with procedural `createTree` fallback — decorative only, not `TreeLifecycle`. Herd species spawn one or two juveniles alongside an adult per a species-specific chance table.

**Real cave habitat (plan fauna-019):** a small fauna-owned `AnimalHabitatBinding` (`src/fauna/animalCaveHabitat.ts`) ties a stable `habitatId` to a real, world-owned walk-in cave (`source: { kind: 'cave', caveId }`) — resolved through `Caves.resolveHabitat(caveId, entityHeight)` (a cave-scoped semantic/traversal contract added to `createCaves.ts` / `src/world/caves/caveHabitat.ts`, O(1) via the existing cave-id map, no scan, no presentation dependency) into a `CaveTraversalDescriptor`: a standable interior chamber (`home`), the cave's own entrance, and a deterministic route between them built from `CaveTopology`'s segment graph (never a navmesh). `createFauna()` accepts `caveWorld`/`caveHabitats` and resolves this once per matching `PersistentOccupantDecl.habitatId` (fauna-018's existing registry — no second persistence layer), spawning that occupant with an `AnimalAgent.cave` context instead of a `PreySpawner` position. That context is the one ground/movement seam every mode shares: `snapY()` prefers this cave's own stateless floor (`Caves.queryGroundIn`) and wall containment (`Caves.resolveHorizontalIn`, sized from `def.modelHeight`/the same capsule-radius scale the fallback mesh already uses), falling back to ordinary surface `sampleHeight` once genuinely outside it — never the player's hysteretic `Caves.queryGround`. A committed trip whose destination is on the surface first walks the interior route to the entrance (`AnimalTrip.phase === 'traveling'`), then normal surface travel; the return leg reverses it, via a small non-persisted per-agent cursor (`advanceCaveRoute`) rather than a nearest-point search, so a folded `adventure`-cave route can never send the resident back to an earlier waypoint. Nothing about this is quest-specific — `caveId` plus the deterministic topology/heightfield are authoritative. **Dungeon residents (plan fauna-027):** each accepted `dungeon` cave gets a pure `buildDungeonResidentsPlan()` pass at `WorldBundle` composition time (no dungeon logic inside `createCaves.ts`): chamber-scoped `habitatId` (`<caveId>:dungeon-chamber:<nodeId>`), `occupantKey: resident`, deterministic initial population (entrance-adjacent excluded, one reserved eligible chamber, 75% optional rolls, at least one predator, bear/wolf weighted species), and `AnimalHabitatBinding.source.homeNodeId` so `resolveCaveTraversal(..., { homeNodeId })` can place dry homes (pool chambers prefer `CaveUndergroundPool.shorelineApproach`). Shallow underground pools expose cave-local environmental drink (`WaterSourceRef.kind: 'environmental'`) and infinite diet-gated fish (`environmentalFood` / `dietAcceptsItem`) through `AnimalCaveContext` + cached `homeToPoolRoute` topology pursuit — no fish items, no dungeon AI, no extra save schema. Persistent occupants use the same fauna-018 slot/tombstone rules; `createFauna()` skips all persistent declarations when `?debugDisableSystems=animals`.
...
**Water traversal** classifies the physical water answer (owned by terrain — see [water.md](./water.md)) into dry/wading/swimming/impassable for a given species, using the species' own scale to derive wading depth rather than a new per-species field. `isWalkable` is **physical ability**, shared by autonomous wander, local-grid pathfinding, and mounted player-driven movement — walkability can never diverge between free-roaming and ridden movement. Autonomous **route preference** is a separate scoring/cost layer (plan fauna-029): casual wander/trip/needs treat swimming as expensive (and refuse swimming destinations unless `waterAdapted`), while flee/chase/lead/mounted keep swimming as ordinary locomotion. Drowning damage applies only while actually swimming and stamina-exhausted, and stops immediately on reaching shore.

## Corpse, decay, and rabies lifecycle

Death (`takeDamage()` → collapse, `onDeath` fires once regardless of cause) starts a decay timeline in **world days** from `deathAtDays` (`elapsedDays`): **fresh** (4 world-hours) → **rotting** (36 world-hours) → **bones** (72 world-hours), then removal at 112 world-hours. Knife-harvest is gated to the fresh phase only, unharvested, unburied; harvested remains linger 2 world-hours from `harvestedAtDays` (a separate rule). A rotting corpse gets lightweight presentation-only decay FX and saps stamina from nearby live fauna within a small radius — a genuine, if small, ecosystem-consequence hook reusing the existing stamina resource rather than a new status-effect system. Once the linger window ends, the corpse is actually removed from the world — NPCs also dispose after 2 world-days (`npcPostDeath.ts`; see [npc.md](./npc.md)). Predator food-claim (`claimedBy`) and household sanitation reservation (`cleanupClaimantNpcId`) are mutually exclusive transient locks on that same corpse state — sanitation is not a second corpse lifecycle, and it is not persisted (plan settlements-npcs-029). An unresolved stray-livestock corpse (plan fauna-024) is the same dead `AnimalAgent`; inspect sets `corpseInspected` as a quest flag and does not harvest or extend linger past the natural world-time clock.

**Animal-to-carcass feeding** (plan fauna-036) reuses the ordinary `sourceTarget` + `actionTimer` forage action: carcass duration is `CARCASS_EAT_DURATION_SEC` (8 s) via `sourceActionDuration()`, longer than ordinary food (`EAT_DURATION_SEC` = 3). Hunger relief and `markFoodConsumed()` stay atomic at completion. Higher-priority branches that call `cancelSourceTarget()` (attack/flee/fire/scare) abort mid-feed and release the claim with the corpse unconsumed; `player-ignore` / `npc-ignore` continue `updatePredator` so proximity alone does not cancel feeding. Partial eat progress is not persisted on the corpse.
...
A stray episode is not only quest-materialized (plan fauna-025): each household-owned individual classifies its own sustained displacement locally, every tick, with no settlement-wide scan. A short flee never latches — only continuous distance past the ordinary wander band for a bounded grace window starts the same authoritative `beginStrayState()` the quest path uses, at the animal's current (already-displaced) position rather than a fresh teleport. Guard dogs are excluded (a `dog-guard` chase can legitimately range far without being lost), as is a currently player-led animal. Once stray, an autonomous home-return commitment reuses the existing single `AnimalTrip` slot (`kind: 'home-return'`) rather than a second movement system: it claims that slot ahead of an ordinary periodic water trip, defers completely under active threat/scare or the existing ad hoc hunger/thirst pursuit (both already sit above `wander()` in `updatePrey()`'s tail, so no new priority tier was added), and retries on a short cooldown if its bounded destination probe (near the stored origin) fails — never a teleport, never a guaranteed return. Arrival inside the canonical return radius clears the episode through the same `fauna-024` predicate regardless of who is watching, so a live self-return during an active lost-livestock quest is reported as `returned` without player proximity, leading, or interaction.

**Household dogs** (fauna-011) are livestock-role `AnimalAgent`s (`AnimalDef.role: 'livestock'`, meat diet, `fleeRange: 0`), not a parallel type and not predators — they eat meat but never hunt, and carcass-seeking is gated by role rather than a dog-specific flag. Guard resolution (`dogGuard.ts`) is recomputed fresh every tick: a wolf attacking this dog's own household wins inside a wide home radius (an own-household attack now means either a wolf's committed *human* target **or** its committed *livestock* prey belonging to the dog's household, plan fauna-026 §7 — see below); a wolf attacking another settlement inhabitant may be assisted inside a tighter radius; a distant/unrelated wolf never becomes a chase target. Contextual bark is a separate, cooldown-gated stimulus (active guard, then a nearby wolf howl, then a stranger at the house) and never chains dog-to-dog. Idle pest-chase of settlement rats is a third, lower contract — only reached once guard/needs/attraction claimed nothing this tick, and home-bounded so a dog never leaves its yard hunting vermin.

**`AnimalDef.role` does not line up 1:1 with "is this economically livestock"** — a source worth stating plainly since it's easy to assume otherwise: of the 7 kinds `settlement/livestock.ts` spawns, only `horse`/`donkey`/`cow`/`dog` use `role:'livestock'`; `sheep`/`chicken`/`rooster` keep `role:'prey'` so they get ordinary prey flee/threat behaviour on their own. "Is this a wild-pool or livestock-pool prey candidate" is therefore a question of *which caller-supplied collection the object came from* (`Fauna`'s own `agents` vs. `Settlement.livestock`/detached livestock), not a `role` check — see the predator/livestock encounter set below.

**Wild predator ↔ household/player livestock encounter set (plan fauna-026):** a wild predator's prey acquisition (`AnimalAgent.resolvePreyTarget()`) now has two independent candidate sources instead of one: the existing wild `others` pool (`role:'prey'`) and a second, caller-supplied `huntableLivestock` list — every live agent from currently loaded settlements' `.livestock` plus `SettlementsManager.getDetachedLivestock()`, deduplicated by `animalId`. The two pools are never merged: livestock is never appended to wild `others`/`agents`, and `Fauna.update()` never ticks it (livestock keeps ticking exactly once, through `tickSettlementLivestock()`/detached livestock, as before). `gameLoop.ts`'s small pure `src/app/faunaEncounterComposition.ts` (`buildHuntableLivestock`) assembles this view after `SettlementsManager.update()` (so this frame's stream-in/out is reflected) and before `Fauna.update()`; `Fauna.update()`'s `huntableLivestock` parameter is required, not optional, so a production/test caller can't silently omit it. The resolver compares the nearest wild candidate against the nearest livestock candidate and commits to whichever is closer (a tie breaks on stable `animalId`, never `Math.random()`); a committed livestock target is revalidated against membership in that tick's `huntableLivestock` (not just `health.dead`), so a settlement stream-out drops the commitment immediately instead of chasing/hitting a disposed reference. Chase/attack/damage/death reuse the existing `updatePredator()`/`attack()`/`takeDamage()`/`onDeath` path verbatim — this is a composition fix, not a second combat system. `AnimalAgent.huntingPrey()`'s existing `{ animalId, ownerHouseId }` shape needed no contract change: `ownerHouseId` is derived from the live target's own `AnimalOwner` regardless of which pool it came from, so a real household-sheep kill now flows unchanged into `gameLoop.ts`'s `threateningAnimals` → `shepherdFlock.ts`'s `senseOwnedFlockThreat()` and into `dogGuard.ts`'s own-household guard tier (`DogGuardWolfCandidate.preyTarget`, alongside its pre-existing `npcTarget`).

**Systemic attraction** (fauna-023) generalizes trap-bait lures into one world-source snapshot (`AnimalAttractionSource`: food / blood / trapBait) built once per fauna pass from `PlacedTraps`, `DroppedItems` and live blood traces. Pure `resolveAttractionTarget()` (`animalAttraction.ts`) scores compatible candidates; `AnimalAgent.pursueAttraction()` moves after needs but below flee/chase/fire/guard. Dropped food is consumed via `DroppedItems.consume()` (not player `collect()`); blood uses bounded investigate memory; trap capture stays in `createPlacedTraps`. Diet gates items; `role === 'predator'` gates blood (dog excluded); scavenging gates spoiled meat and late carcass phases.

**Rats** are a settlement-local pressure/nuisance mechanism implemented as plain `AnimalAgent` instances, physically hosted in `src/settlement/` but fauna-owned by convention. Carrying capacity is food-driven (`(household food + settlement food) / a food-per-pressure constant`, clamped to a small cap). Damaged infested storage adds a temporary `max(normal + 3, 7)` pressure bonus; repairing storage removes that bonus without deleting live rats. An intact infestation nest enables extra replenishment toward that target (at most one spawn per half-day reconciliation, with a deterministic dog-pressure roll); destroying the nest stops that extra replenishment but does not extinguish ordinary food-driven recovery. Dogs no longer subtract from the population target — they hunt rats physically and reduce infestation replenishment by 10% per living dog (floored at 50%). Player melee/ranged combat collectors read `settlement.rats` alongside livestock and wild fauna (plan fauna-021); a rat is not a full unarmed animal interactable. Rats drain real food through the exact same atomic primitives every other consumer uses (household/economy withdraw), gated by a deterministic hashed per-rat roll so outcomes don't depend on frame timing:
