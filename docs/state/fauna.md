...
A household livestock individual can enter a durable stray/displacement episode (`animalStray.ts`, plan fauna-024) without changing `animalId` or `AnimalOwner`. Origin/home is stored on the animal; a one-shot domain reposition picks a walkable point outside the yard (predator pressure is a score penalty, not immunity). Survival assist is a gated flee-range/speed modifier on that episode only. Return is a world predicate (alive + inside the stored origin radius). Dead strayed livestock stays the same corpse `AnimalAgent`; ordinary short corpse TTL is deferred until inspect or a two-day simulation cap. Temporary lead reuses `animalLead.ts` without transferring ownership.
