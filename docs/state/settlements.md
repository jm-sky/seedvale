...
| S7 | Autorytatywny stan NPC — `health`/`stamina`/`vigor`/`needs`/`physicalInjury`/`helperAssignment`/`activePlan`/`postDeath`/`personalInventory` — jest persystowany w `SaveData.npcStates` (opcjonalne, sparse, od planu persistence-001; `postDeath` od npc-010; `personalInventory` od settlements-npcs-026). `phase`/`pendingAction`/pathfinding/`combatIntent`/noszony roboczy `Inventory` (`carried`) **nie** są — resetują się przy każdej rekonstrukcji, więc Continue nie przywraca pełnej *wykonania*. Osobiste belongings nie resetują się wraz z `carried`. | zob. [npc.md](./npc.md#persistence), [persistence.md](./persistence.md) |
...
- Produkcja recipe (`ProductionDef`: stock-only, item-only i mixed) commituje się synchronicznie przez `executeProduction` (`economy/productionExecutor.ts`) z istniejącego seamu `npcWork.ts` (zakończenie work action). `SettlementEconomy.produce()` jest cienkim stock-only adapterem — nie szuka gospodarstw ani item ownerów. Hunter craftuje strzały z `Household.items` (branch → 1 arrow, beam → 8 arrows, ten sam executor). Textile Worker przetwarza wełnę z `Household.items` (`4 wool → 12 wool_material`, ten sam executor; preview nie rezerwuje wełny, completion rewaliduje live state). Brak osobnego production managera, ticka ani persystowanego stanu produkcji; wynik żyje w ownerach (stock osady / itemy gospodarstwa).
