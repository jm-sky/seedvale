...
- `travel` — optional generic spatial continuity checkpoint (plan npc-029, reusing settlements-npcs-019 off-screen duration math). Holds last detailed position plus optional off-screen `execution` (`departedAtDays`/`arrivesAtDays`). Absent execution means a live `NpcAgent` owns progress. Used so reconstruction does not reset an accompanying NPC to home spawn.
...
**Idle-duty dispatch / accompany (plan npc-029, extended by npc-030/npc-031).** After higher-priority pressures have been arbitrated, `beginIdle()` runs a small idle-duty seam: an escort service-fulfilment check first (`tryResolveEscortService()` — duration elapsed or shared player+NPC destination arrival ends the contract + commitment exactly once, before the accompany executor would otherwise silently keep claiming the idle slot forever), then active accompany commitment, then Work Contract, then a voluntary-join proposal (`tryProposeVoluntaryJoin()`, plan npc-031 — deliberately last, so it never jumps ahead of an accepted Work Contract or an already-active accompany commitment), then ordinary schedule/social/profession/idle. Accompany is not a `NeedId` and not an `activePlan` Goal — urgent hunger/thirst/vigor/weather/combat/flee interrupt the current follow/stay *action* through the existing `resetInFlightAction()` path without deleting the commitment, and the next idle cycle resumes it. Follow uses shared `resolveFollowHysteresis()` (same primitive as fauna Follow/Lead) and ordinary `goTo` navigation against the player's current position; Stay holds a world-space anchor recorded on the commitment. A new incompatible Work Contract/accompany pair is rejected at creation/acceptance, not re-arbitrated every tick. Settlement stream-out hands spatial progress to `NpcAuthoritativeState.travel` using the same off-screen duration estimate as transport (`estimateOffscreenTravelDays`); stream-in reifies at the interpolated position rather than home spawn. Ending the commitment returns the NPC to ordinary schedule/place behaviour. A `serving` escort assignment with no live commitment (a reconstruction/save-load gap) repairs itself through the same public `startAccompany()` seam rather than staying stuck. Debug: `seedvale.debug.npc(id).startAccompany()` / `.setAccompanyMode()` / `.endAccompany()`.
