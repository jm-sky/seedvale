...
**Expedition assignment** (`src/world/expeditionAssignment.ts`, plan settlements-npcs-027) is a separate world-owned group commitment: a sponsoring settlement, a destination ref, and an ordered trio of existing NPC ids. It is not stored on `NpcAuthoritativeState`. Lifecycle is `forming` → `provisioned` → `ready`; travel/off-screen movement is out of scope here. Dispatch-time household safety (`canDispatchParty`) is not a runtime profession-minimum manager. Provisioning moves real items from `SettlementEconomy.items` into each member's `personalInventory` as one all-or-nothing transaction. Persist as sparse `SaveData.expeditionAssignments`.
