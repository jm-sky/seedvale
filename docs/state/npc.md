...
**Profession work** (dispatched by role) is a pure-function-per-profession table, not a shared scoring engine: miner (ore gathering, skips `Household` entirely — ore carries straight to `SettlementEconomy` in the NPC's own inventory; the extraction step uses the shared Strength physical-work duration rule, the chained stockpile `deposit` travel step does not), hunter (arrow crafting during the `work` block; the hunt expedition itself is pressure-driven off the `food` need, not profession work), farmer (harvest before plant, plant only from a real household-held seed, around a shared `CultivationAnchor` that can be the settlement garden or a supplied Player-built garden), fisher (a real dock, the same deterministic catch rule the player uses), guard (a fixed patrol route, always succeeds), trader (own-household surplus → economy first, falling back to a world-owned `TransportOrder` that physically collects one concrete food `ItemKind` from another same-settlement household), blacksmith (gated on a household-held whetstone — confirmed currently dormant, since nothing yet supplies one), shepherd (owned-flock shearing/deposit/herding), textile worker (household `4 wool → 12 wool_material` through the shared production executor when the owning `Household.items` already holds the wool; missing local input is a normal blocked/fallback stand, not a WorkContract). Strength is not applied globally to `rollWorkDurationSec()` — harvesting, fishing, patrol, trading, arrow crafting, textile processing and generic workplace stands stay Strength-neutral. A planner returning nothing falls back to a generic idle-at-workplace stand: profession is a preference, not the only way to act.
