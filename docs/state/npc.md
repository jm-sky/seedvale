...
**Work contracts (evaluation only).** The commitment record itself (lifecycle, employer, group commitment, assignments) is owned by the world/player side — see [player-systems.md](./player-systems.md)'s Work Contracts section for the full mechanism. The NPC side is: a pure evaluator scores a contract opportunity (expected remaining share's reward + role suitability − travel cost − expected-work-duration cost − schedule-conflict penalty, compared against zero) only from an NPC's own idle branch (never pre-empting a real need); once accepted, `NpcAgent` drives that NPC's assignment through discovery → accept → travel → work-bout, resolving `{ contract, assignment }` fresh every decision cycle rather than keeping a second copy. Several NPCs may hold independent assignments on the same contract; one worker sleeping, fighting or dying does not pause the others. Work bouts against a buildable target go through that buildable's own actor-neutral work-contribution seam — the same one the player's own busy-channel work calls — so player and NPC labour on the same target are never tracked as two separate progress fields.
