...
**Profession work** (dispatched by role) is a pure-function-per-profession table, not a shared scoring engine: miner (ore gathering, skips `Household` and transient `NpcAgent.carried` — successful extraction writes into the world-owned resource-site `Inventory` for that deposit id; the extraction step uses the shared Strength physical-work duration rule; a later Trader `TransportOrder` moves the ore into `SettlementEconomy` stock, plan settlements-npcs-021), hunter (arrow crafting during the `work` block; the hunt expedition itself is pressure-driven off the `food` need, not profession work), farmer (harvest before plant, plant only from a real household-held seed, around a shared `CultivationAnchor` that can be the settlement garden or a supplied Player-built garden), fisher (a real dock, the same deterministic catch rule the player uses), guard (a fixed patrol route, always succeeds), trader (resume any active `TransportOrder` first, then a world-owned food `TransportOrder` when the settlement has uncovered food shortage and a same-settlement household — including the trader's own — has uncommitted food surplus, then remote ore from a resource-site inventory when blacksmith iron/coal stock is short, then wood still uses own-household direct deposit — plan settlements-npcs-020/021), blacksmith (gated on a household-held whetstone — confirmed currently dormant, since nothing yet supplies one), shepherd (owned-flock shearing/deposit/herding), textile worker (household `4 wool → 12 wool_material` through the shared production executor when the owning `Household.items` already holds the wool; missing local input is a normal blocked/fallback stand, not a WorkContract). Strength is not applied globally to `rollWorkDurationSec()` — harvesting, fishing, patrol, trading, arrow crafting, textile processing and generic workplace stands stay Strength-neutral. A planner returning nothing falls back to a generic idle-at-workplace stand: profession is a preference, not the only way to act.

**Physical goods transport** (`src/world/transportOrder.ts`, plan settlements-npcs-018) is a world-owned commitment record for a single concrete `ItemKind` moving `source → carrier → destination`. It is not a cargo owner and not a logistics tick: NPC actions still execute pickup/travel/unload. First vertical slice is Trader cross-household food collection. Carrier cargo, once claimed, is authoritative NPC-owned state — `NpcAuthoritativeState.transportCargo` (`src/settlement/npcState.ts`, plan settlements-npcs-019) — distinct from `personalInventory` (belongings) and transient `NpcAgent.carried` (other professions' brief work payload); it survives `NpcAgent` dispose/recreate, `WorldBundle` rebuild and save/load. Active/non-terminal orders persist as `SaveData.transportOrders`. Execution ownership is exactly one of detailed (the carrier's own live action/movement flow) or off-screen (`TransportOrder.execution`, set only for an already-`in-transit` order whose carrier's settlement streams out) — never both; off-screen arrivals resolve through `src/world/transportOffscreen.ts`'s `resolveOffscreenTransportArrivals`, reusing the same transactional pickup/unload seam as detailed execution, at bounded checkpoints (settlement stream transitions, time-skip completion) rather than a per-frame tick. An order abstracted before pickup (still `assigned`) carries no execution metadata and simply resumes ordinary pickup once a live carrier exists again.
