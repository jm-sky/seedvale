...
**Work contracts (evaluation only).** The commitment record itself (lifecycle, employer, group commitment, assignments, wage claims) is owned by the world/player side — see [player-systems.md](./player-systems.md)'s Work Contracts section for the full mechanism. The NPC side is: a pure evaluator scores a contract opportunity (expected remaining share's reward + role suitability − travel cost − expected-work-duration cost − schedule-conflict penalty, compared against zero) only from an NPC's own idle branch (never pre-empting a real need); once accepted, `NpcAgent` drives that NPC's assignment through discovery → accept → travel → work-bout, resolving `{ contract, assignment }` fresh every decision cycle rather than keeping a second copy. After useful work stops, a payable claim is a bounded local opportunity: if the player is nearby the NPC approaches through ordinary `goTo` navigation and the shared dialogue surface offers Pay. Several NPCs may hold independent assignments on the same contract; one worker sleeping, fighting or dying does not pause the others. Work bouts against a buildable target go through that buildable's own actor-neutral work-contribution seam — the same one the player's own busy-channel work calls — so player and NPC labour on the same target are never tracked as two separate progress fields.

**Idle-duty dispatch / accompany (plan npc-029).** After higher-priority pressures have been arbitrated, `beginIdle()` runs a small idle-duty seam: active accompany commitment first, then Work Contract, then ordinary schedule/social/profession/idle. Accompany is not a `NeedId` and not an `activePlan` Goal — urgent hunger/thirst/vigor/weather/combat/flee interrupt the current follow/stay *action* through the existing `resetInFlightAction()` path without deleting the commitment, and the next idle cycle resumes it. Follow uses shared `resolveFollowHysteresis()` (same primitive as fauna Follow/Lead) and ordinary `goTo` navigation against the player's current position; Stay holds a world-space anchor recorded on the commitment. A new incompatible Work Contract/accompany pair is rejected at creation/acceptance, not re-arbitrated every tick. Settlement stream-out hands spatial progress to `NpcAuthoritativeState.travel` using the same off-screen duration estimate as transport (`estimateOffscreenTravelDays`); stream-in reifies at the interpolated position rather than home spawn. Ending the commitment returns the NPC to ordinary schedule/place behaviour. Debug: `seedvale.debug.npc(id).startAccompany()` / `.setAccompanyMode()` / `.endAccompany()`.
