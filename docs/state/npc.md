...
**Physical goods transport** (`src/world/transportOrder.ts`, plan settlements-npcs-018) is a world-owned commitment record for a single concrete `ItemKind` moving `source → NpcAgent.carried → destination`. It is not a cargo owner and not a logistics tick: NPC actions still execute pickup/travel/unload. First vertical slice is Trader cross-household food collection. Orders are runtime-only in 018 — they are not in `SaveData` and are not carried across a `WorldBundle` rebuild, because `carried` itself is still transient.
