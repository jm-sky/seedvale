...
`PlacementPreviewActions` is the single construction-preview lifecycle for chest, tent, fires, standing torch, palisade, bedroll, platform, well, work-contract and residential-house targets. The ghost (`world/placementPreview.ts`) draws a circle or box footprint from `PlacementPreviewResult.footprint`; house previews also show a front/entrance marker on the local `-Z` edge, the same front as `residentialBuildingApproachLocal()`. Domain clearance stays the existing `footprintRadius` circle. Placeables that declare `supportsRotation` freeze a 45°-snapped object yaw at preview start (`F`/`G` or overlay buttons step it, with `[F]`/`[G]` labels on the buttons for pointer devices); camera look still moves the aim point. Confirm re-resolves the site with that yaw and re-runs domain validation — the preview result is never authoritative.

## Camp rest quality

`src/app/campRest.ts` is a pure module, not a manager. `CampRestContext { hasBlanket, hasWarmFire, tentCondition, bedrollCondition, platformCondition }` is resolved once at rest/inspection time from the existing `PlacedTents` / sleeping utilities / `PlacedFires` lists via `src/app/campRestSnapshot.ts` (a fire must be lit and within a warmth radius; village fires don't count — town rest is already full). Tent/bedroll/platform condition is a persisted 0..100 lazy weather-driven value — tents decay slower than exposed utilities, and a tent's current shelter factor (`condition/100`) reduces utility weather exposure smoothly. `campRestQuality()` interpolates tent shelter between the no-tent and full-tent endpoints of the matching blanket/fire pair (blanket only 0.55 < blanket+fire 0.75 < tent+blanket 0.8 < full camp 1.0 at 100% tent), then adds a bedroll bonus whose raised-platform factor interpolates 0.75..1.00 from supporting-platform condition. Survival still closes up to 60% of the remaining gap. Inspection `[R]` on a player tent opens the existing `FlavorDialog` with the same snapshot/quality rest uses. A tent whose bedroll/platform fall inside the existing camp radii is one composed `camp` interactable (plan items-player-022) — Tab/gaze no longer cycles the three physical parts; standalone bedroll/platform outside a tent stay independent. Inspection rows are structured (`formatCampInspectionDetails`) so positive `+N%` contributions render as a positive tone; repair actions are offered per detected component through the existing `campRepair.ts` quote/work path. Tent/bedroll/platform inspection can also start or resume a repair episode (plan items-player-019): `src/items/campRepair.ts` quotes materials/work/capability/effort from the resolved condition; the world owner stores optional `RepairProgress` and exposes `startRepair` / `contributeRepairWork`. Tent and bedroll require a carried `textile_repair` capability (`sewing_kit`); platform uses `wood_chopping`. Materials commit once at start from inventory + nearby dropped items; Busy Action is one work bout; interruption keeps accepted work; resume re-checks the capability and does not re-consume materials. Completion writes condition 100, clears `repair`, and resets the degradation anchor. Active tent repair blocks packing. Portable tents are instance-backed (`TentItemInstance.id == PlacedTent.id`, condition `0..100`); packed tents do not weather-degrade. Repair XP is awarded only for accepted work. `restoreNeedsFromSleep(needs, quality)` can only fail to fill the bar, never lower it below what it already had; stamina is always fully refilled regardless of quality. `app/actions/restActions.ts`'s `onSleepFinished()` owns applying the outcome. Quick Action `Śpij na biwaku (8h)` is the original blanket bivouac rest; `Rozbij pełny obóz` is a single-active intent (`app/actions/fullCampIntent.ts`, same pattern as cook-meal) that reuses/places tent → platform → bedroll → fire through the existing placement preview and Busy Actions, without a `CampManager` or persisted camp membership.

## Settlement lodging (plan 168)

"Nocuj w mieście" (Quick Actions, `RestVariant: 'town'`) no longer sleeps the player in place. `src/settlement/lodging.ts`/`lodgingResolver.ts` define a small `LodgingOption` contract (`bed`/`friend`/`paid`/`hay`) and one pure preference policy (`resolveBestLodging`: bed > friend > paid > hay, quality desc, distance asc, stable id tie-break) — `restActions.ts` resolves the best loaded-settlement option, then walks the player to its `approachPoint` (by writing into the same shared `KeyState`/`LookState` `PlayerController.update()` already reads each frame from `gameLoop.ts`'s `tickLodging()`, no second movement system) and only starts the existing Sleep/time-skip on arrival, re-validating availability first. A manual movement-key press cancels the walk. `friend` reads `NpcAgent.household`/`Household.homeId` and the existing `PlayerSocialLookup` relation level (no new friendship registry); `hay` anchors on the settlement's existing `landmarks.garden`; `bed` and `paid` are structurally defined but currently always empty — no bed provider before plan 169, no inn/hotel economy in the repo. A paid offer (once one exists) requires explicit confirmation (`onConfirmLodging`/`onCancelLodging`) and charges `Inventory`'s existing `'coin'` stock exactly once, before the walk is armed. Lodging quality (`high`/`normal`/`low` → `1`/`0.75`/`0.45`) feeds the same `restoreNeedsFromSleep()` camp rest uses, via its own `pendingLodgingQuality` — no Survival XP, no second regeneration model.

The walk is cancellable at any time — `Esc` (already first in `App.vue`'s keydown chain, ahead of pause) and a HUD "Anuluj [Esc]" button (`LodgingWalkOverlay.vue`, driven by `isLodgingActive()`) both call the same `abortRest()` (plan `ui-input-005`). `tickLodging()` also runs a stuck-movement watchdog: it tracks the closest XZ distance to `approachPoint` reached so far (pure calculation in `advanceLodgingProgress`), and if that distance hasn't improved by a meaningful epsilon for `LODGING_STUCK_TIMEOUT_SEC` (~12 s) — e.g. a house collider blocking the straight-line path — it recovers by placing the player directly on `approachPoint` via the existing `PlayerController.setPosition()` (same seam `startTentRest` uses), then runs the exact same arrival/completion path (`completeLodgingArrival`) as a normal arrival. No global collision bypass, no noclip, no second sleep-start path.
...
Since plan items-player-017, placement creates a real but unfinished torch (`completedWork: 0`, always `unlit`) — `[E]` runs one active-work bout (`workOnStandingTorch`, `STANDING_TORCH_WORK_SESSION_HOURS`/`_SEC`, same partial-credit-on-cancel shape as well work) through the actor-neutral `StandingTorches.contributeWork` seam, shared with NPC Work Contract execution (`ai/NpcAgent.ts`'s `pursueBuildableContract`). Construction is independent of `lit`: only once `completedWork` reaches `STANDING_TORCH_REQUIRED_WORK` (`isStandingTorchConstructionComplete`) does `[E]` switch to the original ignite prompt (`fire_starting` capability, no-op if already lit) — an unfinished torch cannot be lit and never enters the active flame/light list. The visual height scales cheaply with progress (`Object3D.scale.y`, no per-frame work). A pre-plan save has no `completedWork` field; the v5→v6 migration defaults it to `STANDING_TORCH_REQUIRED_WORK` (already complete).

A completed torch burns for six world hours (`burnUntilDays = igniteNow + 6/24`) and then becomes unlit. Expiry uses `dayNight.elapsedDays`, so normal play, time skip and save/load share the same deadline. A pre-plan lit save has no ignition timestamp — the v27→v28 migration turns it unlit rather than granting a free six hours. Portable `wooden_torch` / branch fuel stays a separate real-time `PlayerTorch` system.

Palisade segments (plan items-player-010, extended by items-player-017) share the identical construction model: `world/palisade.ts`'s `PalisadeSegmentRecord` gained `completedWork`, `PALISADE_REQUIRED_WORK` (a fixed constant, not per-record, since every segment costs the same) is the single requirement authority, and `world/createPalisades.ts`'s `contributeWork` is the actor-neutral seam. An unfinished segment still reserves its footprint for placement/snapping (plan §10's "planned footprint ≠ completed functional barrier") but its OBB collider is registered only once construction completes, so NPC pathing doesn't treat a half-built fence as a barrier. `[R]` removal stays available at any progress and now also invalidates any active Work Contract on that segment (`WorkContracts.findByTarget`), so a hired NPC never keeps travelling toward a segment the player tore down.

## Player-built residential houses (plan settlements-005)

`world/residentialBuilding.ts` + `world/createResidentialBuildings.ts` are the persistent house construction collection. Quick Actions "Zbuduj małą chatę" / "Zbuduj średnią chatę" enter the shared placement-preview pipeline (rotated rectangular footprint). Placement does not spend materials. Each work-bearing stage (`foundation` → `structure` → `roof`) is material-gated: `[E]` supplies the current stage's existing `ItemKind`s once, then Player/NPC `contributeWork` advances only that stage and never spills into the next. Slope rejection creates a covering `TerrainPreparationRecord` instead of flattening silently. Completion of a Player-initiated house registers `home:residential:${id}` and derives high-quality `owned_house` lodging through the existing rest path — no physical bed in v1, no Household. Completed houses are non-removable; cancelling an unfinished house recovers only unused current-stage materials and invalidates its Work Contract. NPC labour does not change ownership. Lodging options are derived, never persisted.
...
`Przygotuj teren` is a larger, world-space flattening job with ordinary integer metre sizes from `2×2` through `9×9` (discrete selection, not a free-form slider). Quick Actions enters a preview mode (`app/actions/terrainPreparationActions.ts`) — a world-space grid follows the player's aim (mouse wheel/`+`/`-` resize, `,`/`.` target-height step of 0.25 m), validated every frame against the immutable-once-confirmed sample snapshot: water margin, the 6 m per-sample deformation cap, mountain-rock requiring a pickaxe, and the shared `evaluateGroundPlacement` ground-suitability check. Confirming (`[E]`) creates a compact `TerrainPreparationRecord` (`terrain/terrainPreparation.ts`) — `id`/`center`/`size`/`targetHeight`/immutable `originalHeights`/`requiredWork`/`completedWork` — and a temporary marker (`world/createTerrainPreparations.ts`).

Work itself reuses `world/timeSkip.ts` as a generic "work N hours" primitive (the same mechanism "Czekaj"/sleep already use), started by `[E]` on the marker: `requiredWork = max(1 game-hour, area × avgAbsHeightDelta × workScale)`, tool speed is additive (shovel mandatory, +5% knife, +10% pickaxe) and shortens the actual hours skipped. Every frame the skip is active, `completedWork`/terrain heights are re-derived from the immutable `originalHeights` via `progressiveHeight()` — never accumulated, so interruption (Esc, damage/starvation — reusing the existing `interruptLongActivityOnDamage` entry point, no new vigor-threshold interrupt) and resume, or a save/load mid-work, reproduce identical terrain deterministically. `createTerrainPreparations`'s constructor re-seeds `ChunkManager.applyExactHeights` for every restored active record so partially-modified terrain is correct immediately after load, before any chunk streams in. Completion is a single actor-neutral transition (player or NPC/shared work): exact target heights stay in the existing terrain-modification overlay, the active record/marker is removed, and a compact `CompletedTerrainPreparation` (`id`/`center`/`size`) is recorded and persisted. `querySiteInfrastructure()` (`world/siteInfrastructure.ts`) is a read-only, on-demand aggregation over those completed areas, usable Player wells (`isWellWaterAvailable`) and live Player gardens inside a caller-provided centre+radius — it reports world facts, not colony readiness booleans.

Settlement Farmer work (`ai/npcProfessionWork.ts`) plants/harvests around a shared `CultivationAnchor` (position + actual cultivation radius). Settlement garden pads produce that contract at prop-build time from `GardenScale`; a Player garden projects through the same shape (`PLAYER_GARDEN_PLANT_RADIUS`) so a future settlement can farm an existing Player plot without copying garden or crop state.

## Animal traps (plan 141, extended plan fauna-014)

`src/world/animalTraps.ts` + `createPlacedTraps.ts` are player-placed world objects in `WorldBundle`, the same persisted-record shape as tents/fires — there is no separate trap manager. Bought from the home trader, set down through the shared ground-placement evaluator, then `[E]` picks up (broken) or disarms (active) directly, or opens the arm choice panel (`placed`); `[R]` picks up a non-armed trap. Detection is a pure function of the trap's quality (higher → lower detection floor) rolled against a deterministic per-`(trap, animal, attempt)` roll; an evasion sets a runtime-only cooldown, not persisted (wild fauna isn't persisted either). Species coverage is now trap-kind-aware (`isSpeciesTrappable(trapKind, kind)`, `animalTraps.ts`'s declarative `TRAP_SPECIES_COMPAT`): `simple` catches rabbit/fox/deer/boar, `good` additionally catches `stag`/`wolf`; `bear` and every domestic/livestock kind stay out of the player trap system entirely. A catch only kills through the existing `AnimalAgent` damage/collapse/death path and deactivates the trap — it leaves an ordinary, un-harvested corpse; the player still knife-harvests it normally. Weather wears down an *armed* trap lazily, charged per completed weather cycle, not per frame.

Bait (plan 159, reworked plan fauna-014) is an optional food item that both cuts detection chance (`TRAP_BAIT_DETECTION_CUT`) and now actively lures compatible fauna: `[E]` on a `placed` trap opens a `FlavorDialog` panel (`gatheringActions.ts`'s `openTrapArmDialog`) offering "Uzbrój bez przynęty", "Uzbrój z przynętą…" (a sub-panel listing only bait-capable items actually carried, via the shared `BAIT_ITEM_PRIORITY`) and "Zabierz pułapkę" — the old auto-selected-first-bait-item behaviour on arm is gone. The chosen item is removed from inventory only after the trap is armed and has accepted the bait (`armTrapWithBait`), so a failure never needs a compensating add-back. An active+baited trap exposes a cheap `PlacedTraps.activeLures()` snapshot (`TrapLureDescriptor`: trap id/kind/position/bait); `gameLoop.ts` computes this once per fauna pass and forwards it into every `AnimalAgent.update()` call. `AnimalAgent`'s `resolveLureTarget()` (pure, in `AnimalAgent.ts`) picks the nearest lure whose trap kind can catch this species (`isSpeciesTrappable`) and whose bait this species' `AnimalDef.diet` actually accepts (`dietAcceptsItem` — `wolf`/`fox` now carry the same meat `diet.items` as `dog`, inert for hunger since a predator's `findFoodTarget()` always takes the carcass branch, read only for this bait-attraction check); `updatePredator`/`updatePrey`'s new `pursueLure()` step runs after flee/threat and real hunger/thirst pursuit (`pursueNeeds`) but before plain wander, so attraction never pre-empts a live chase or an urgent need and always loses to a stronger behaviour. Reaching the trap's own (smaller) `triggerRadius` still goes through the unchanged detection/capture roll — attraction only draws the animal closer, it is never a guaranteed catch. Per-item stats (durability, detection floor, price) are in [CATALOG.md](../items/CATALOG.md).
...
Lifecycle: `available → advertised → active → settling → completed` (plus `cancelled`/`invalidated`). Worker execution (`accepted → travelling → working`) and per-assignment wage claims (`payment_due → paid` / `unpaid` / `uncollectable`, or `released` with no wage) live on `WorkContractAssignment` records owned by the contract — never as a second copy on `NpcAgent`. `rewardCoins` is the group ceiling; each frozen claim is `floor(workCompleted * rewardCoins / committedWork)` and the sum of frozen claims never exceeds that ceiling. A successful Pay moves `coin` items from the player `Inventory` into that NPC's authoritative `personalInventory` via `transferInventoryCount`. Creating a contract (`bundle.workContracts.create()`, a work-share preset of 25/50/75/100%, a worker-count preset of 1/2/3, plus a reward preset that is the **total** group ceiling, not a per-worker wage) does not advertise it — the player must physically walk to a settlement's notice board and post it (`postWorkContract`) before any NPC can discover it. The posting stays discoverable while a work slot and useful group work remain; `committedWork` is one group total, never multiplied by worker count. A payment-due worker does not occupy a work slot.
