...
A well becomes a usable `WaterSource` once its `well`-stage body is finished (`isWellWaterAvailable`) — the roof is protection, not activation; `[R]`'s requirements panel gains "Napij się"/"Napełnij pojemnik" buttons once water is available, even mid-roof-construction. `isWellCompleted` (`stage === 'roof'` and done) still gates full construction/NPC-contract completion and now also whether the well's `WaterSource` carries a `consumptionRisk`: an uncovered well has a 50% chance, rolled at the moment of direct drink (never at fill/build time), of `-1..2 HP` and `-5 Vigor` through the existing `HealthState`/`VigorState` primitives (`app/actions/survivalActions.ts`'s `drinkFromWaterSource`); filling a container doesn't carry the risk forward, same limitation `undrinkable` ocean water already has. Once the `roof` stage's work is done, the well becomes a plain `well`-kind water source with no risk — the exact settlement-well drink/fill path otherwise, no parallel water system. NPCs may also target any water-available player well (roof optional) for their own water-fetch when it's closer than the settlement's own well.
