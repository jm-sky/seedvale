...
- Visuals: `world/weatherVisuals.ts` dims sun/ambient/hemi and adjusts fog on top of day/night. `storm` is a canonical weather type (plan world-026) with heavier rain presentation and a short lightning flash overlay that does not mutate day/night state. `world/weatherParticles.ts` renders rain/snow as GPU-driven `THREE.Points` (per-particle fall/drift computed in a shared shader from a fixed-at-creation attribute + `uTime`; JS only updates a few uniforms) — density follows weather intensity and the `quality.lodScale` graphics preset. The rain/snow shape contract (thin vertical streak vs. procedural flake mask) is [GRAPHICS.md](../architecture/GRAPHICS.md) G13, not restated here. Deterministic lightning/thunder events live in `world/lightningEvents.ts` (bounded per weather cycle; same event identity for flash, delayed thunder, and livestock scare).
