...
- Chunk rocks/logs and visible iron/coal/gold deposits use GLB templates with procedural fallbacks. Ordinary surface ores still come from the resource grid; the abandoned mountain mine adds landmark-owned gold definitions (plan world-018) into the same `ResourceDeposits` runtime, with Cave V2 floor Y for interior nodes and optional explicit finite reserve.
- Procedural landmarks (`monolith`/`stoneCircle`/`smallRuins`) share one placement resolver (`resolveClassicLandmarkPlacement` in `chunkEnvironment.ts`) between streamed `computeChunkEnvironment()` and unloaded `ChunkManager.findLandmarkNear()`. Unloaded lookup samples height/road tint/ridge/moisture through `createLocalTerrainSampler` instead of materializing a full `ChunkTileData`. Settlement cemeteries (`kind: 'cemetery'`) are assignment-driven from settlement topology (`src/terrain/cemeteryAssignment.ts`) then placed by `cemeteryPlacement.ts` (dedicated fringe or shared `SM` corridor, plus a rare abandoned wilderness roll). Physical gates (water/slope/roads/footprint) stay in `chunkEnvironment.ts`. Authored expedition `ruins` are a cheap containment check, distinct from procedural `smallRuins`. `WorldLocationCatalog.cemeteryForSettlement()` consumes cemetery assignment; it does not own nearest-landmark search.
...
- Visuals: `world/weatherVisuals.ts` dims sun/ambient/hemi, tints fog, and overlays Sky.js turbidity/rayleigh on top of day/night (`applyWeatherSkyOverlay` — storm raises turbidity and lowers rayleigh; rain is a weaker step of the same; `dayFactor`/sun angles stay weather-independent). `storm` is a canonical weather type (plan world-026) with heavier rain presentation and a short lightning flash overlay that does not mutate day/night state. Grass wind amplitude (`grassWindAmpFor` → shared `uWindAmp`) scales the existing tip sway; bounds are padded for the 1.8 storm ceiling at placement, not rebuilt. `world/weatherParticles.ts` renders rain/snow as GPU-driven `THREE.Points` (per-particle fall/drift computed in a shared shader from a fixed-at-creation attribute + `uTime`; JS only updates a few uniforms) — density follows weather intensity and the `quality.lodScale` graphics preset. The rain/snow shape contract (thin vertical streak vs. procedural flake mask) is [GRAPHICS.md](../architecture/GRAPHICS.md) G13, not restated here. Deterministic lightning/thunder events live in `world/lightningEvents.ts` (bounded per weather cycle; same event identity for flash, delayed thunder, and livestock scare).
- Weather → gameplay coupling: the live per-frame `WeatherState` is threaded, once per frame (never recomputed per consumer), from `gameLoop.ts` through `SettlementsManager`/`Settlement.update()` into every `NpcAgent.update()` call, where it competes as the `seekShelter` decision candidate (`ai/weatherPressure.ts` — see [npc.md](./npc.md#decision-architecture)). `isRainWeather()` is the shared rain-exposure helper (`rain` and `storm`) for wetness, gardens, and `computeRainExposureDays()`. Separately, `computeRainExposureDays()` (a bounded-lookback derived function over the same weather history) feeds player-garden hydration and sleeping-utility condition decay — two independent lazy consumers of one deterministic weather function, never a second weather simulation. Thunder scare is a short fauna impulse (`fauna/animalScare.ts` → existing `AnimalAgent.fleeFrom`), not a quest or stray flag.
- `world/clouds.ts` (plan world-terrain-001, cloud variety per plan world-terrain-014) is a bounded/recycled pool (`CLOUD_COUNT = 28`) of `THREE.Sprite` billboards, player-XZ-following, sky-level and independent of `WorldBundle`. Each sprite belongs to a rendering-only `light`/`dense` category (its own texture list + height/scale/drift-speed ranges); `cloudCategoryWeightsFor(weather, season?)` is a pure weighted-selection function used only when a sprite is first assigned or recycled (never a global reassignment on weather change), so a weather transition reads as a gradual population shift. `cloudAppearanceFor(weather, elev)` separately drives coverage (`sprite.visible` threshold) and a shared-material tint/day-night multiply — unaffected by category selection. Coverage lerps from the clear baseline by intensity, then `storm`/`rain` clamp to a high floor (0.92 / 0.72) so a weak or debug-forced cycle still reads as overcast rather than half-empty.
...
- Wetness/snow cover (`world/weather.ts`'s `computeSurfaceWeather`, plan 133) are pure, bounded-lookback derived values from `(seed, elapsedDays)` — no per-chunk state, no save field. Rain and storm raise wetness; snow accumulates and melts (temperature-gated) back into wetness.
...
- River channel carving is stage 3 of [Terrain shaping stages](#terrain-shaping-stages) above (`applyRiverChannel`), consuming the declared ford projections `settlement/roadNetwork.ts` supplies (`terrain/riverFord.ts` owns the shaping maths only). Stage 2's road corridor blend also suppresses its own height/tint contribution inside a declared bridge's deck footprint (`terrain/roadBridge.ts`'s `isOnBridgeDeck`, `ChunkTileParams.bridgeProjections`, plan world-terrain-033), so a road never carves a causeway through the river a bridge spans.
...
- `settlement/roadNetwork.ts` routes river-aware: its A* prices every candidate **edge** against canonical river water through the one evaluator (`settlement/roadRiverCrossing.ts`), and the resolved `RoadRoute.crossings` are the canonical `ford`/`bridge` records. No terrain, renderer or runtime stage may decide a crossing exists or reclassify one; terrain projects a declared ford, and `roadNetwork.bridgeSpecOf()`/`bridgesNear()` project a declared bridge into a deterministic `RoadBridgeSpec` — V1 procedural deck presentation (`world/createBridge.ts`, owner-chunk instantiated by `ChunkManager`) and the shared movement-ground query (`ChunkManager.sampleSurfaceGround`/`sampleBridgeDeck`) player/NPC/fauna movement all read. Details in [water.md](./water.md).
...
Coarse world-location classification (lakes, mountain peaks, cemeteries, …) consumes this domain's terrain sampling surface (`sampleContinentalnessAt`/`sampleFloorAt`/`sampleHeightAt`/`sampleMountainRidgeAt`) but is a separate, persisted-progression domain, not part of terrain generation — see [world-locations.md](./world-locations.md). Cave interiors are not heightmap features: Cave V2 (`createCaves()`, `src/world/caves/`) is a streamed SDF interior owned by `WorldBundle.caves`, sited by `pickLargeCaveSites()`. Mouth recess carves are `'system'` terrain modifications (replayed every world build, never persisted). Cave *retained* worldgen — the accepted archetype/topology manifest plus each cave's heightfield, content anchors and dungeon pool — can now hydrate from the disposable `caves-v2` persistent worldgen cache: `createWorldBundle()` awaits that read immediately before the still-synchronous `createCaves()`, a miss or malformed record simply regenerates (per cave, or wholesale for a bad manifest), and presentation still streams normally. Cave placement, topology, archetype guarantees and anchor rules are unchanged by it. Gameplay ground inside a cave is a derived SDF column index (`caveSdfQuery.ts` / `Caves.queryGround`). Wall colliders and third-person camera occupancy use that index's strict occupancy (`caveSdfColliders.ts` / `Caves.occupancyAt`), not the `CaveVolume` proxy — that adapter remains for `definitions()` / location catalog until plan world-terrain-008 Milestone B5. Presentation meshes are extracted off the main thread (dedicated cave worker) and do not gate collider relevance.

## Persistence

Terrain, hydrology, and weather/season are deterministic reconstructions of `(seed, region params / elapsedDays)` — never persisted. The only persisted terrain-adjacent state is a sparse delta over that deterministic base: `SaveData.terrainModifications` (player-caused edits only; system-caused ones like cave carving are filtered out at save and re-derived) and `SaveData.resourceDeposits` (remaining yield per deposit). A separate, disposable persistent worldgen cache (`persistence/worldgenCacheDb.ts`, `(seed, namespace, version, fingerprint) → payload`) is structurally distinct from `SaveData`; its current namespaces are world-location coarse classification, abandoned-cemetery materialization (see [world-locations.md](./world-locations.md)), Cave V2 retained worldgen (`caves-v2`, `src/world/caves/caveWorldgenCache.ts`), canonical chunk tiles (`chunk-tiles`, `src/terrain/chunkTileWorldgenCache.ts`) and inter-settlement road routes (`road-routes`, `src/settlement/roadRouteWorldgenCache.ts`). `createWorldBundle()` activates `road-routes` hydrate into `roadNetwork.ts`'s session `routeCache` before the first chunk request; consumers stay synchronous, a runtime-computed route wins over in-flight hydrate, and a miss just runs A*. Canonical `RoadRoute.crossings` round-trip with the route — fords and bridges are not stored separately. `ChunkManager.ensureLoaded()` still builds the full `ChunkTileParams` (roads, village paths, clearings, rivers, cemeteries) every time and uses its fingerprint for a direct primary-key lookup: a hit skips only the tile worker job, a miss runs the worker exactly as before and writes the result back fire-and-forget. Both paths hand runtime a `cloneChunkTileForRuntime()` copy, so the cached base tile is never mutated by the terrain modifications, cutouts and lifecycle overrides that are replayed downstream unchanged. The remaining terrain caches (`riverTileCache`, `ChunkMeshDataCache`) are in-session only and not persisted. See [persistence.md](./persistence.md) for the full classification.

## Entry points

```text
src/settlement/roadNetwork.ts
src/settlement/roadRouteWorldgenCache.ts
