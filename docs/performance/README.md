...
Fauna's shared `AnimalAgent` measured at ~20 ms/frame CPU (wild ~14 ms + livestock ~7 ms) on benchmark 018, split roughly evenly between the behaviour (movement/steering) and life/presentation halves, with ~28 of 28 wild agents running expensive behaviour every frame while ~0.2 were high-priority. Plan `fauna-028` addresses that by **reducing how often existing work runs**, not by adding a spatial index: one shared importance/cadence policy inside the single shared `update()` lowers the behaviour and presentation sections' rate for ordinary/far animals while sensing, targeting, decision and the whole needs/timers/lifecycle tick stay at full rate. See [fauna.md](../state/fauna.md#architecture) and [performance-and-workers.md](../architecture/performance-and-workers.md#fauna-simulation-granularity). The shared coarse spatial-query mechanism above is still the right answer for the O(N²) pressures and is explicitly *not* what fauna-028 did.

NPC category wall-clock is `SettlementsManager.update()`. While the perf monitor is active, `src/perf/agentCpuDiag.ts` splits that total into crowd pass, NPC agent updates, livestock, rats, social pairing, settlement streaming (`recheck`), settlement maintenance (storage visuals / doors / torches / signposts), and a remaining `unattributed` remainder. Do not treat a large `unattributed` as a new subsystem — inspect the current `update()` path first. The fauna block additionally reports cadence counters (full-rate vs reduced-cadence agents, behaviour/presentation executions per frame) for both the wild and livestock channels; `expensive behaviour agents/frame` counts actual executions, so it is the headline before/after number for plan `fauna-028`.
