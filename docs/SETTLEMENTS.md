...
- Jedna rodzina = jedno gospodarstwo = jeden dom (`household.ts`'s `householdIdFor(settlementId, familyIndex)`), 1:1 z `def.families`/`homePlaces` w `createSettlement.ts`. `HouseholdRegistry` żyje na `SettlementsManager` (jak `EconomyRegistry`) — stream-out/in reużywa te same gospodarstwa; nie jest w `SaveData`. W przeciwieństwie do `EconomyRegistry`, `HouseholdRegistry` nie ma też carry-mechanizmu przez in-session rebuild `WorldBundle` (np. zmiana ustawień terenu) — stock gospodarstwa cicho resetuje się do świeżych, jitterowanych wartości startowych, podczas gdy `SettlementEconomy` jest w tej samej sytuacji przenoszona (`carriedEconomies`); znany gap, patrz plan `195`'s audit notes / `docs/plans/LOOSE-ENDS.md`.
