...
**Created:** 2026-08-18
**Status:** `verification needed` 🔍 — audit found the generic transport contract already implemented: wood (`chop → deposit`), water (`waterDuty`'s well-queue → home `deposit`) and ore (`mine → deposit`) already share the same chained `NpcPlannedAction` (`goTo` source → `onComplete` extract → `next: { kind: 'deposit', ... }` → `onComplete` credit destination) pattern, with `this.carried` `Inventory` already used for ore and closure-captured amounts for wood/water giving the same atomic conservation; food already gathers straight into `Household.stock`. Per the [implementation notes](./2026-08-18--156--npc-household-and-settlement-storage-logistics-implementation-notes.md) (§12, "do not refactor if already generic"), none of that was touched. The actual gap was physical observability: added a household storage container (small crate, one per house yard, `props.ts`'s `buildSettlementProps` → `landmarks.householdStorages`, zipped with `Household` by index in `createSettlement.ts` → `Settlement.householdStorages`) and one settlement storage container (larger crate next to the wood stockpile, `landmarks.settlementStorage`) as new `Interactable` kinds (`householdStorage`/`settlementStorage`, `src/interaction/Interactable.ts`), read-only `[E]` stock view via `resolveInteraction.ts` querying `Household.stock`/`.water` and `SettlementEconomy.query()` live (settlement view excludes `water` — it's unused seed-only stock, the real reserve stays on `Household.water`). No new manager/inventory/economy system; props are pure presentation, rebuilt from the persistent `Household`/`SettlementEconomy` registries on every stream-in. Technical verification green (`tsc`/lint/build/test, 1045 tests); no browser verification yet.
