...
**Status:** `verification needed` — zaimplementowane zgodnie z opisem poniżej (`src/settlement/roadNetwork.ts`, `src/settlement/minorLocations.ts`, rozszerzenia `chunkHeightmap.ts`/`biomeColors.ts`/`NpcAgent.ts`), brak jeszcze wizualnej weryfikacji w przeglądarce. **Update (2026-08-08):** [village-generation.md](./2026-08-08--031--village-generation.md) dodało obok tego kod `villageSegmentsNear` (`roadNetwork.ts`) łączący w jednym przebiegu clearing/regionalne wygładzanie terenu/ścieżki do domów — nowa siostrzana funkcja, opisany tu system dróg/ścieżek między osadami działa bez zmian.
**Created:** 2026-08-07
