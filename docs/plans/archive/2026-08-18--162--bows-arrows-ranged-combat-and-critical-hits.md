...
- **No models/sounds exist yet** for bows/arrows (`docs/assets/MODELS.md` M50/M51 — procedural fallback wired) or bow draw/release (`docs/assets/SOUNDS.md` S22 — currently silent); arrow hit/kill reuse the existing melee hit/kill sounds (no dedicated arrow-impact clip).
  - **Update (2026-08-21):** M50/M51 wired — `short_bow`/`hunting_bow`/`long_bow`/`arrow` GLBs (Medieval Weapons Pack / Quaternius, `Bow_Wooden`/`Bow_Wooden2`/`Bow_Golden`/`Arrow`, OBJ→GLB via `obj2gltf` + `gltf-transform` meshopt) now ground- and held-wired; `broadhead_arrow`/`war_arrow` share `arrow.glb` (no visual damage-tier distinction). Held-grip TRS reuses the pre-existing procedural-fallback `HELD_ATTACH` values and is **not yet manually verified in-hand** in the browser. S22 (bow draw/release sound) is still unaddressed — no matching clip found in local staging on a quick pass.
  - **Update (2026-08-21b) — real hold-to-draw/release-to-fire input, sounds, animation stand-in, ammo toasts:** Ranged fire moved from `combat/rangedLifecycle.ts`'s pure `drawTime` timer to a real press/release input: `E`/LMB/mobile-`E` are now one shared "primary attack/use" action (`Keyboard.ts`'s new `interactReleased` + `consumeInteractRelease()`, `MouseLook.ts` gained `mousedown`/`mouseup` writing into the same `KeyState`, `TouchChrome.vue`'s `E` button gained `@pointerdown`/`@pointerup`/`@pointercancel`/`@pointerleave`) — melee stayed press-to-attack, unchanged. `rangedLifecycle.ts`'s `start()` gained an opt-in `manualRelease` flag (default `false`, so NPC callers via `NpcAgent.ts` are provably unaffected) plus a `release()` method: releasing at/after `drawTime` fires (unchanged `fireReady`/projectile pipeline), releasing earlier cancels the shot (no ammo spent, stamina already drained is **not** refunded — same forfeit as the existing modal/downed interrupt paths). S22 wired: `bow-draw.ogg`/`bow-release.ogg` (`public/sounds/`, provenance not yet recorded — see `public/sounds/README.md`) play on draw-start and on the actual fire frame (not on an early-release cancel) via new `playActionBowDraw`/`playActionBowRelease` (`src/audio/actionSounds.ts`). Animation: audited both requested sources for a dedicated bow-draw/release clip compatible with Adventurer's own rig — neither works. `Universal Animation Library[Standard]`/`Universal Animation Library 2[Standard]` (`_temp/Models/packs/`) ship a UE-mannequin-style skeleton (`neck_01`, `index_01_l`, …) with no bow/arrow clips at all among their 43+43 animations; Mixamo's `Pro Longbow Pack` (`_temp/Models/animations/`) has exactly the needed semantic clips (`standing draw arrow`, `standing aim overdraw`, `standing equip bow`, …) but is rigged `mixamorig:*` — both are a different skeleton than Adventurer's own (`Hips`/`Abdomen`/`Wrist.R`/…), and Three.js has no built-in retarget (consistent with the earlier `docs/plans/archive/2026-08-14--asset-audit-3d-models.md` finding that UAL↔current-character retargeting was rejected as too costly). Reused Adventurer's own already-bundled, same-rig `Idle_Gun_Pointing`/`Gun_Shoot` clips as stand-ins instead (`PlayerController.ts`'s new `beginRangedDraw()`/`endRangedDraw()`/`playRangedRelease()`, mirroring `beginMeleeAttack()`'s existing pattern) — a real bow clip remains a nice-to-have, not tracked as a numbered model-backlog row since it's an animation gap on an existing model, not a new GLB. Ammo toasts: "Brak strzał." → "Brak strzał w ekwipunku." (both the pre-draw check and a new check at fire time, for the edge case where ammo is depleted between draw-start and release); a new "Zostało {count} strzał" toast fires after a successful shot showing the remaining stack count of the arrow kind actually fired (no Polish plural-form agreement — matches the existing lack of declension elsewhere, e.g. `+{count} Gałąź`).
