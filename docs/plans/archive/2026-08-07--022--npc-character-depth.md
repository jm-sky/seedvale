...
**Status:** `done` — wszystkie sekcje (Character DB, role, Big Five, traits, wspólny `HealthState`, ekran Mieszkańcy) zaimplementowane, zielone na `tsc`/`lint`/`build`/`vitest`, i potwierdzone w przeglądarce przez użytkownika (2026-08-08, patrz „Do przetestowania”). Przy okazji dodano `vitest` jako test runner projektu (nie było w oryginalnym zakresie planu) — `src/ai/dialogue.test.ts`, `src/ai/Needs.test.ts`, `src/shared/HealthState.test.ts`, `src/fauna/HealthState.test.ts`.
**Created:** 2026-08-07
