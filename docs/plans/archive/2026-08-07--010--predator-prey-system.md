...
- **Corpse linger (feedback po review)**: ciało po śmierci zostawało niewidoczne od razu (`mesh.visible = false` w `takeDamage`), znikało "za szybko". Zmieniono: ciało zostaje widoczne (zamrożona poza — mixer przestaje się aktualizować) przez `CORPSE_LINGER_SECONDS` (obecnie **60s**; wcześniej 8s), dopiero potem `createFauna` / settlement livestock je usuwa (`AnimalAgent.readyToRemove()`). Paski HP ukrywane przy `collapse()`. Z łopatą w ręku: `[E] Zakop zwłoki` → busy → `bury()`. **Deferred:** scavengowanie zwłok przez drapieżniki (pożywienie). Przy okazji naprawiono bug: `updateSpawners` liczył martwe-ale-jeszcze-widoczne zwłoki jako "żywy prey w pobliżu", co mogło blokować respawn przy dłuższym lingerze — dodano `!a.isDead()` do filtra.
