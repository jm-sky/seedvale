...
## Implementation decisions (deviations from the plan text)

- **Announcement representation:** no new `ItemKind`, dropped-item entry, or player-inventory slot was added. Creating a contract (Quick Actions → Budowa → "Zleć budowę", aim at a spot, pick a reward) is a single action that both creates the `available`/`not_posted` `WorkContractRecord` and — since the player is the one writing it — implicitly hands them the announcement; there is no separate "take announcement" step. The plan's core invariant (creation never publishes, never assigns a board) is preserved: the player must still travel to a settlement and interact with its notice board to post.
- **Advertisement storage:** kept as fields on `WorkContractRecord` itself (`advertisement`, `postedBoardId`) rather than a separate `ContractAdvertisement` object stored on the board. A board's "what's posted here" view (`WorkContracts.postedAt(boardId)`) is a live query over contracts, never a duplicated list — satisfies §8/§12's "advertisement references the contract, never duplicates it" without a second persisted collection.
- **Target validity/invalidation:** `WorkContracts.invalidateTarget(id)` is exposed as a manual API for later phases to call; no automatic detection exists yet, since nothing else in the current game can invalidate a construction-contract target this phase. Covered by a direct unit test (manual call → cleanup), not an automatic trigger.
- **Cancellation UI:** a new Quick Actions category ("Zlecenia") lists the player's own non-terminal contracts with a direct one-press cancel — not a flag interaction. The plan only requires cancellation to be possible, not proximity-gated; this reuses the existing Quick Actions list mechanism instead of adding a second interaction candidate/gaze target for the flag.
- **Notice board visual:** reuses the existing procedural `createSignpost()` prop (`settlement/settlementStructures.ts`) as a placeholder — no dedicated notice-board asset exists yet (same "procedural until authored" convention as the blacksmith's parked-asset comment in `settlement/props.ts`). `SettlementLandmarks.noticeBoard` holds its deterministic position; `world/workContract.ts`'s `noticeBoardId(settlementId)` derives its stable identity.
- **Contract flag visual:** a small inline procedural pole+flag (`world/createWorkContracts.ts`), not a separate prop module — it has nothing to cache/clone, unlike GLB-backed props (palisade/standing torch).
