...
- `LOOSE-ENDS.md` 2026-09-10: `PlayerController` still feeds the raw surface `sampleHeight` to `applySlopeMovementConstraint` inside caves (harness uses `withCaveFloorFallback`). Unchanged by this cutover. **Fixed 2026-09-11** — `PlayerController.slopeHeightSampler()` wraps the surface sampler in the same `withCaveFloorFallback` the harness uses, keyed by the stateless `caveOccupancy` floor at the player's own position (not `caveGround`, whose hysteresis is per-frame), with `Caves.sampleFloor` as the Y-blind probe sampler (new `CaveFloorSampler` dependency wired in `createApp`). Identity outdoors; regression pinned in `src/player/caveSlopeSampling.test.ts`.
