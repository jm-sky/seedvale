...
- Hook: `src/app/actions/groundActions.ts::checkHiddenFindDig()`. After `resolvedHiddenFindSpotIds.add(match.spotId)`, cemetery-only: `BadgeManager.recordGraveDisturbed()`, then one exposure roll. `villageNearest` runs once per grave and supplies both loot `size` and `settlementId`. Missing `id` → badge/loot still resolve, no reputation write, no global fallback.
