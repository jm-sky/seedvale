...
Stage D (props/wagon/support/crate/lantern presentation, `PointLightBudget`-gated lanterns) can assume: cave chests exist and are ordinary containers; `caveTreasureContainerSpecs` is the composition precedent to follow for any future anchor-role materialization; the remaining anchor roles (`wagon`/`support`/`crate`/`lantern`) are already resolved by Stage B and unused by Stage C — read them via the same `caves.contentAnchors()` call already made in `worldBundle.ts`.
