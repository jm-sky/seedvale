...
- Thunder audio uses `worldAudio.playOnce()` with volume from clip band + simulated distance + cave muffle. Clips `thunder-very-close.ogg` / `thunder-mid.ogg` / `thunder-distant.ogg` (S28 `wired`) are picked from `simulatedDistanceM`. Storm rain reuses the rain loop louder; storm wind reuses the existing wind loop.
