...
**Update 2026-09-02 (step 6, later same day):** implemented exactly as designed in §5.6, plus two corrections the design didn't anticipate (both required for correctness, not scope growth — see below). `AnimalAgent.ts`'s `npcThreat` gate (`update()`, formerly `:2010`) now reads `this.def.role === 'predator' ? this.resolveNpcTarget(nearbyNpcs) : null` — `resolveFrenzyNpcTarget()` renamed to `resolveNpcTarget()`, its target-commitment mechanism (the `if (this.npcTarget) {...}` re-validate-else-resenses shape) untouched. `this.frenzyNpcTarget` renamed to `this.npcTarget` (field + `getDebugInfo()` key) — no external consumer referenced the old name (checked: only `AnimalAgent.ts` itself). `faunaDecision.ts`'s `FAUNA_BEHAVIOUR_PRIORITY`/`isBehaviourValid` needed **zero changes** — the `npc-attack`/`npc-ignore`/`npc-flee` validity predicates already required `!input.frenzied` (steps 2-5 built the general shape in anticipation, per §5.6's framing), only the `AnimalAgent`-side gate was narrow.

Two corrections beyond the gate flip:

1. **Village exclusion (§5.6's "Village avoidance" open question, resolved).** `senseNpcThreat()` now skips NPC candidates inside `isNearVillage()`'s radius when `!this.frenzied` — mirrors `updatePredator()`'s existing "live prey inside the village is not huntable" rule (same file, no new mechanism). Without this, a non-frenzied wolf could resolve `npc-attack` on an NPC standing inside a settlement and `chaseNpc()` straight into it, contradicting the "wild animals avoid settled ground" invariant every other predator/wander/wander-target path already enforces (plan 044 §2.3/§2.4). The exclusion applies only at *acquisition* (inside `senseNpcThreat`), not on every tick of an already-locked `npcTarget` — consistent with `resolveNpcTarget()`'s existing "don't re-apply `playerNoticeRange` once locked" precedent, and deliberately not fixed further here (see the new `LOOSE-ENDS.md` entry: a locked target that later walks into a village keeps being chased in). A frenzied predator keeps considering every candidate, since it's already explicitly willing to enter a village (`moveTowardStrategicVillage`).

2. **Cache separation (found during implementation, not anticipated by §5.6).** `refreshThrottledHumanIntent()` (player) and `refreshThrottledNpcIntent()` (NPC) shared `humanDecisionTimer`/`cachedHumanIntent`/`cachedAggressionRoll` — safe pre-step-6 only because the two guards (`sense.playerActive && role === 'predator'` vs. `npcThreat && !frenzied`) could never both hold (`npcThreat` implied `frenzied`, so the second guard's `!frenzied` always failed). Step 6 breaks that: a non-frenzied predator can have both a visible player and a visible NPC in the same tick, so both refreshes would run, both would decrement the *same* timer (double-decrementing it), and whichever ran second would overwrite `cachedHumanIntent` with the other's result — corrupting the next throttled read for up to `HUMAN_DECISION_INTERVAL_SEC` (0.2s). This tick's own branch selection is unaffected (player-* outranks npc-* in `FAUNA_BEHAVIOUR_PRIORITY`, so the correct branch still wins the tick the corruption happens on), but a subsequent tick could briefly read a stale/wrong-context intent. Fixed by giving `refreshThrottledNpcIntent` its own `npcDecisionTimer`/`cachedNpcIntent`/`cachedNpcAggressionRoll` (same idiom, separate fields — not a new abstraction). Every existing `this.humanDecisionTimer = 0` reset site (rabid gate, fire-avoid, frenzy-beeline, predator-normal, prey-normal, `takeDamage()`'s retaliation reset) got a paired `this.npcDecisionTimer = 0` for the same "force an immediate re-score after a state transition" reasoning, symmetric with the player-side resets.

`senseNpcThreat()` also switched `Math.hypot` to a squared-distance compare (implementation notes' explicit ask below, §5.6's "Perception cost" bullet) — necessary now that every loaded predator scans `nearbyNpcs` every tick, not just the ~1 frenzied wolf. Did **not** add throttling to the scan/target-acquisition itself (only the *intent* computation is throttled, as before): the codebase's existing precedent for a caller-bounded per-tick scan is `senseEnvironment()`'s own `litFires` loop, which is unthrottled — only the probabilistic *roll* (`cachedPerceptionRoll`) is cached/staggered, never the scan. Adding a separate perception-refresh timer here would be a new abstraction for this one case (guard rail: don't do that) and would make `npcTarget` acquisition laggy in a way nothing else in this file is.

`decideNpcResponse()` needed no scoring changes (per §5.6 point 3) — verified `fireNearby: this.frenzied ? false : sense.fireNearby` already resolves the "NPC at a lit campfire" case sensibly: `sense.fireNearby` is the predator's own proximity to the nearest lit fire (not the NPC's), but since `FIRE_AVOID_RADIUS` (11) exceeds `playerNoticeRange` (10, see `senseNpcThreat`'s doc), a predator close enough to notice an NPC standing at a campfire is essentially always also within `FIRE_AVOID_RADIUS` of that same fire — so `npc-flee`/`npc-ignore`/`npc-attack` already factor in `FIRE_FEAR` correctly, without needing the NPC's own distance to the fire. This matches §5.6's predicted outcome ("likely `npc-flee`... verify this reads correctly in play") — not independently re-verified in a browser this session, see below.

`faunaDecision.test.ts` gained 4 new cases (`decideFaunaBehaviour — npc-008 step 6` describe block): a non-frenzied predator resolving `npc-attack`/`npc-flee`/`npc-ignore` purely from `npcIntent`; frenzy forcing `npc-attack-frenzied` even when `npcIntent` would have said `flee`/`ignore` (proves frenzy skips scoring, not just wins a tie); `npcThreat` with no resolved `npcIntent` yet falling through to `predator-normal` rather than a stale `npc-*` branch; `npc-flee` still outranking `fire-avoid` for a non-frenzied predator (same priority as the already-tested frenzied case). All pre-existing parity tests (steps 2-5) were left untouched — `pnpm run test` is 2375 passed (2371 + 4 new), `npx tsc --noEmit` and `pnpm run lint:fix` both clean.

**Not done this session — verify in a browser before calling npc-008 fully `done`:** frenzied wolf still reaches a settlement and attacks via `npc-attack-frenzied` (regression check — should be visually unchanged from before step 6); a non-frenzied predator near a hungry/aggressive state can now `npc-attack` a nearby NPC and combat resolves correctly; `npc-flee`/`npc-ignore` read as sensible reactions (not jittery, not stuck); a wolf near a settlement does not chase an NPC standing inside it; a wolf near a lit campfire with an NPC nearby resolves to a coherent single reaction (not oscillating between fire-avoid and npc-flee).
...
**Implemented 2026-09-02 — see the "step 6" update note above the table of contents for what actually landed (including two corrections this design section didn't anticipate). The design below is kept as-written for history.**
