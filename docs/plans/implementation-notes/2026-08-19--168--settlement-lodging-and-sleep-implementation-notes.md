...
## 28. Follow-up (2026-08-28) — player-chosen lodging + hay bale interaction

§26's "Nocuj w mieście" auto-picked the resolver's best option with no player choice, and hay had no real `[E]` target of its own. Both closed as a thin extension of the existing contract — no new interaction system, no second lodging UI:

- **Choice panel** — `startRest('town')` no longer arms movement itself. It now collects every candidate (`collectNearbyLodgingOptions`, the renamed/generalized former `resolveLodgingOption`) and, via the new `RestActionDeps.openLodgingPanel` dependency, opens the existing generic contextual panel (`FlavorDialog`/`openFlavorDialog`, the exact mechanism `ui-input-002` built for the well's `[R]`) listing one `InteractionPanelAction` per `LodgingOption` (`lodgingChoiceLabel()` — place name + quality or price, new in `lodging.ts`). `RestOutcome` gained `'choose'` (dialog already opened as a side effect) and dropped `'confirm'`.
- **Selection commit** — picking an action calls `commitLodgingSelection(optionId)`, which re-collects candidates fresh and classifies the pick via a new pure `lodgingResolver.ts` function, `selectLodgingFromCandidates(candidates, id)` → `{ unavailable | confirm | walk }` (unit-tested in isolation, no `PlayerActionContext` needed). `unavailable` (stale panel, place gone) toasts and does nothing else; `walk` arms movement exactly as `startRest` used to; `confirm` opens a *second* `openLodgingPanel` call (place/price, "Potwierdź"/"Anuluj") instead of the old inline Quick Actions block — same `confirmLodgingPayment`/`cancelLodgingConfirm` commit-once state machine underneath, unchanged, just triggered from a dialog action instead of a Vue-bound `lodgingConfirm` ref. `resolveBestLodging` itself is untouched and still exported/tested — nothing currently calls it at runtime, but per the plan's own §21/§7 it stays as the policy for a future automatic-pick caller.
- **Hay as a real interaction** — a new `Interactable` kind `'hay'` (`Interactable.ts`), built in `buildInteractables()` from the exact same `settlement.landmarks.garden` anchor `collectHayCandidate` already used (never a second coordinate), prompts `[E] Nocuj w sianie`. `resolveInteraction.ts` excludes it (needs `RestActions`, same reason as `tent`/`playerWell`); `gameLoop.ts` dispatches it directly to a new `RestActions.sleepInHay(settlementId)`, which calls `commitLodgingSelection(hayLodgingId(settlementId))` — the new `hayLodgingId()` helper (`lodging.ts`) is now the single source for that id string, shared by the resolver's `collectHayCandidate` and this direct path, so both always resolve to the identical candidate. Since hay is never priced, this always lands on the `walk` branch — same arm-movement/arrival/Sleep path as picking "Stóg siana" from the panel, just skipping Quick Actions.
- **Removed** — `LodgingConfirmView`, `setLodgingConfirm`/`setQuickActionsLodgingConfirm`, `onConfirmLodging`/`onCancelLodging`, and `QuickActionsScreen.vue`'s inline paid-confirm block are gone; the paid flow is fully expressed through `openLodgingPanel` now, so nothing was left duplicated.
- **Not done / open** — same as §27: no live paid-lodging provider to exercise the confirm panel against in actual gameplay; this session verified `tsc`/lint/test(2001 passed)/build only — browser verification (Quick Actions choice panel, hay `[E]` prompt/interaction, no accidental teleport, Esc/cancel mid-walk) is being done by the user directly rather than by this agent.

## 29. Follow-up (2026-08-28) — panel duplicates, hay range, vigor-aware rest cancel

Three playtest regressions from §28, fixed as thin extensions of the existing contract — no new lodging/interaction/rest system:

- **Panel duplicates** — `bed` and `friend` can resolve to the exact same physical house (a `bed`-equipped house whose owning NPC is also `friendly`/`trusted`). `LodgingOption` gained an optional `placeId` (`lodging.ts`) — the existing house-index identity (`${settlementId}:house:${houseIndex}`, the same one `homeIndexFromPlaceId` already derives), set by both `collectBedCandidates` and `collectFriendCandidates` when they resolve to a house. `collectLodgingCandidates` now runs the merged list through a new `dedupeByPhysicalPlace()`, keeping the single highest-priority option per `placeId` (same bed > friend > paid > hay + quality + id ordering as `resolveBestLodging`) and passing through every `hay`/`paid` option (no `placeId`, no known collision today) unchanged. `resolveBestLodging`/`selectLodgingFromCandidates` are untouched — the fix is entirely in what candidate list they ever see.
- **Hay interact range** — the `hay` `Interactable`/lodging-resolver fallback were anchored on `landmarks.garden` (the garden pad center), but `buildSettlementProps` actually places the physical hay-bale mesh `gardenPlotRadius + ~1.4-2.6` away from that point — well outside `INTERACT_RANGE`/`GAZE_RANGE`, so `[E]` could fire (or fail to fire) far from the visible bale. Fix: `SettlementLandmarks` gained `haySpots?: THREE.Vector3[]`, populated with each hay bale's actual placement in the same loop that computes it (`props.ts`). `app/interactables.ts`'s `resolveHaySpot()` (new, pure, unit-tested in `interactables.test.ts`) and `lodgingResolver.ts`'s `settlementLodgingInput()` (gained an optional `playerPosition` param) both anchor on the nearest real `haySpots` entry instead of `garden`, falling back to `garden` only for landmark fixtures that predate the field. No change to `INTERACT_RANGE`/`GAZE_RANGE` themselves or to `gameLoop.ts` — the existing gaze(5)/interact(2.5) two-stage range system already worked correctly once given the real position.
- **Rest cancel vs. vigor** — `Esc` during a night's sleep only ever unlocked via `REST_CANCEL_PROGRESS_THRESHOLD` (85% skip progress), even for a player who started already well-rested. `items.ts` gained `REST_CANCEL_VIGOR_THRESHOLD` (0.5) and two pure functions: `restCancelAllowedByStartVigor(ratio)` (strictly-greater-than-50%) and `canCancelRestNow(progress, vigorAllowedAtStart)` (the `||` of both rules), both unit-tested (`restCancel.test.ts`). `restActions.ts` captures `getVigorRatio(player.needs.vigor) > 50%` once, at the exact `timeSkip.start(..., {fadeStrength: 1})` call for each of the three real sleep starts (lodging arrival, camp, tent) — never for `startWait`'s `fadeStrength: 0.5` skip, so waiting/terrain-prep skips are unaffected — into a new closure flag, reset in `cancelRest()`. A new `RestActions.canCancelRest()` combines the captured flag with live `timeSkip.progress()`; both `abortRest()` (the actual Esc gate) and `gameLoop.ts`'s `updateTimeSkipRestUi()` (the HUD's Esc-button visibility) now call this one function, so the prompt never shows something Esc wouldn't actually honor — `TimeSkipOverlay.vue` itself is unchanged, purely presentational as before.
- **Also (same session, mid-fix user request)** — "Nocuj w mieście" was collecting candidates across *every* `getLoaded()` settlement at once, which could mix in an adjacent settlement's beds/friends/hay near a settlement boundary, and per-settlement hay resolution always picked `haySpots[0]` regardless of player position. `restActions.ts` gained `nearestSettlementInRange()` (nearest loaded settlement within `REST_IN_TOWN_RADIUS`, or `null`); `isNearTown()`/`collectNearbyLodgingOptions()` both now go through it, so the panel only ever lists the one settlement the player is actually near. `lodgingResolver.ts`'s `settlementLodgingInput()` picks the nearest of a settlement's own `haySpots` to the passed player position (falls back to the first entry when position is omitted); `app/interactables.ts`'s `resolveHaySpot()` was similarly changed from "first in range" to "nearest in range".
- Verification this session: `tsc`/lint/test (2013 passed)/build only — browser verification (panel showing one entry per real place, hay `[E]` only firing near the visible bale, Esc appearing immediately at high vigor) is left to the user, per this repo's convention that technical checks aren't gameplay verification.
