...
### 6. Children / final legacy removal

Do not silently scale adult UBC bodies down and call them children.

Before deleting the legacy Modular pool, verify whether the already-owned Universal Base Characters source used by Seedvale provides the Teen male/female bodies documented by Quaternius and whether the existing Fantasy outfit meshes can be composed safely for that body.

If yes, add deterministic UBC child/teen variants and migrate children.

If not, keep legacy models only for children and record that as an explicit remaining boundary. Adult migration must not be blocked by child-body work.

Legacy Modular assets can be removed from runtime preload/model pools only after there are no runtime callers.
...
### Stage E — remaining legacy boundary

1. Inspect Teen UBC compatibility for children.
2. Migrate children if the current owned source assets support it cleanly.
3. Otherwise document children as the sole legacy runtime users.
