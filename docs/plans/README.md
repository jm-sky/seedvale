...
| 💡 `quests-progression-048-hunters-brotherhood-introduction-and-membership.md` | 🟡 | M      | ~~quests-progression-020~~, ~~quests-progression-033~~, ~~quests-progression-034~~                                                                                                                                                                                                         | quests-hunters-brotherhood              |
...
| ◼️ `settlements-npcs-042-vendor-identity-and-settlement-scaled-stock.md`       | 🔴 | M      | ~~settlements-npcs-033~~, ~~settlements-npcs-036~~, ~~settlements-npcs-040~~, ~~settlements-012~~, ~~items-player-040~~                                                                                                                                                                    | economy-goods-flow                      |
...
| Plan | Sprawdź |
|------|---------|
| `quests-progression-055-quest-marker-lifecycle-and-actionability-fixes.md` | New Game: podejść do giverów bez wcześniejszego dialogu — markery widoczne od razu po pojawieniu się NPC. Odejść aż settlement się odstreamuje, wrócić — marker poprawny bez rozmowy. Aktywny quest z samym reminderem/wycofaniem — brak `?`. Prawdziwy talk/action target — `?` zgodnie z dostępnością akcji i cooldownem. Hand-in/report/oferta nadal `✓` / `!`, zwykły active context `…` |
| `settlements-npcs-040-profession-trade-stock-and-hunter-crafting.md` | Household Huntera: Handel z bow/leather/`dried_meat`/`herb`/nadwyżką strzał; kupić starter bow/leather, zamknąć i otworzyć — item nie wraca; zostawić Hunter arrow reserve; praca Huntera uzupełnia arrows/bow tylko produkcją; Woodcutter ≤2 handlowe axes i własna siekiera; Farmer pitchfork/sickle; Blacksmith bounded sword/metal pauldron; forest Merchant `weapons-tools` — bows/arrows osiągalne; save/load po zakupach — sprzedany specialist stock nie odrasta |
| `items-player-040-armor-quality-pricing-and-world-availability.md` | Merchant w SM/MD vs LG/XL: zbroja `Kiepska/Zwykła/Dobra/Mistrzowska` jako osobne wiersze i ceny; kupić `good/masterwork` i zachować jakość; sprzedać różne jakości; save/load — niesprzedany stock i jakość identyczne, sprzedana sztuka nie odrasta |
| `persistence-005-pending-new-save-identity-in-pause-menu.md` | Istniejący save „Przygoda”; New Game „Quest Adventure”; Pause przed Save pokazuje „Quest Adventure”, nie poprzedni slot; po Save ta sama nazwa; Load ma nowy slot; Continue po odświeżeniu nadal poprawna nazwa |
| `items-player-039-ubc-runtime-pauldrons.md` | Asset Browser: cztery skinned pauldron GLB, identity alignment. W grze: Kupiec sprzedaje naramienniki; Załóż `arms` na Peasant/Ranger/Knight; idle/walk/sprint/sword; crouch/jump; clipping; equip/unequip/swap; save/reload; body leather↔chainmail rebind; brown `leather_pauldron`; `?player=adventurer` bez visualu i bez błędu |
