...
| File | Summary | Pri | Effort | Depends |
|------|---------|-----|--------|---------|
| `2026-08-17--141--animal-traps.md` | Pułapki na małe/średnie zwierzęta jako rozszerzenie istniejących systemów (inventory item + placed world object, nie nowy manager): 2 rodzaje (`simple`/`good`), stany rozłożenia/aktywacji/trwałości, detection roll przy podejściu zwierzęcia, złapanie → istniejący lifecycle śmierci `AnimalAgent` i istniejący harvest mięsa; nowy skill `traps` na wspólnej krzywej `xpToSkillValue()`; wymagany model 3D. Wstępny — przed implementacją przeczytać [implementation notes](./2026-08-17--141--animal-traps-implementation-notes.md) (review: snapshot skilla przy aktywacji zamiast referencji do gracza, nazewnictwo stanów, brak per-frame skanu fauny) | 🟡 | M | ~~128~~ |
