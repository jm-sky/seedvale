...
| File        | Notes              |
| ----------- | ------------------ |
| `npc-006-shared-npc-animal-pathfinding.md` | [notes](./implementation-notes/npc-006-shared-npc-animal-pathfinding-implementation-notes.md); new shared `navigation/navigation.ts` (bounded local-grid A* + greedy waypoint simplification, pure/testable, `NavigationQuery`/`AgentProfile`) and `navigation/navigationStats.ts` (request/success/failure/search-time/visited-node/repath/active-path counters, exposed via `window.seedvale.debug.navigation()`) — deliberately request-driven, never per-frame: `NpcAgent`'s existing `npcMovementWatchdog`-triggered `attemptRepath` now tries a real A* route toward the phase's current destination first (`attemptNavRepath`, generalizing the old single-hop `repathTarget` into a `repathWaypoints` queue `steerWithRescue` walks), falling back to the pre-existing blind random-hop bypass only when no route is found; `AnimalAgent`'s predator chase (`updatePredator`) and `fleeFrom` reuse the same `npcMovementWatchdog` module for stuck detection via two independent `NavRescue` instances (`chaseNav`/`fleeNav`, `stepNavRescue`) so a stale route from one mode can never bleed into the other, with a staleness guard clearing a route whose target has since moved far from the live `dest`; normal (non-stuck) steering, `startAction`'s initial goTo movement, and npc-005's target-commitment flows are all untouched — no navmesh, no worker, no path caching per the plan's explicit scope. Obstacle routing, slope/water constraints, waypoint simplification, no-route/bounded-search failure and repath-preserves-committed-target are automated (`navigation/navigation.test.ts`, `navigation/navigationStats.test.ts`); wolf-around-obstacle-while-chasing, NPC-around-obstacle-to-destination, the house/door false-block scenario and no visible frame-time regression at a representative population need browser/performance verification |
| `ai-004-npc-goals-and-persistent-plans.md` | [notes](./implementation-notes/ai-004-npc-goals-and-persistent-plans-implementation-notes.md); new `src/ai/npcPlan.ts` (pure Goal/Plan lifecycle: `secureFood`/`secureWater`/`fulfilWorkDuty`/`obtainWood`, `active`/`interrupted`/`blocked`/`partially_completed`/`completed`/`obsolete`) plugged into `NpcAgent` as `NpcAuthoritativeState.activePlan` (survives `WorldBundle` rebuild, not `SaveData`) — `choose()` establishes/resumes a Plan per decided need before `beginNeed()` runs its existing branches unchanged; completion reuses the same freshly-generated need pressure `choose()` already computes (Goal satisfied by any actor, not just this NPC's own action), `selectAndTraceStrategy()` carries the picked `NpcStrategyId` onto the Plan and blocks/unblocks it when no candidate is available, wood/food/waterDuty's existing `onComplete` callbacks report real progress, and `interruptCurrentAction`/`abandonStuckAction`/`beginCombat` mark the Plan interrupted (never cleared) so a wolf-attack-mid-hunt-style interruption resumes the same Plan afterwards; `NpcInspectionSnapshot`/`NpcTraceEvent` (`plan.created`/`plan.stateChanged`/`plan.progressed`/`plan.completed`) and the NPC Inspector UI extended with a Plan section — multi-action goal pursuit, interruption/resume, blocked/obsolete/completed transitions and diagnostics need browser verification |
| `world-007-hidden-finds-and-reputation-badges.md` | [notes](./implementation-notes/world-007-hidden-finds-and-reputation-badges-implementation-notes.md); generalized the old `settlement/hiddenTreasure.ts` easter egg into `world/hiddenFinds.ts` — deterministic per-landmark dig spots seeded from each landmark's own stable id (`terrain/chunkEnvironment.ts`'s `deriveLandmarkId`), resolved as a side effect of the existing `soil_digging` shovel action (`groundActions.ts`), no new Interactable/marker; cemetery graves are independent multi-spot finds (grave grid reused from `decorProps.ts`'s `cemeteryGraveLayout`, capped by `CEMETERY_MAX_FINDS`, loot scaled by nearest settlement's `VillageSize`), stoneCircle (25%)/monolith (10%) are single-roll landmark treasures; new `badges/badges.ts` `BadgeManager` (Grave Robber/Desecrator/Treasure Hunter/Relic Seeker) and a UI-only community-standing penalty combined with `QuestManager.getPlayerStanding()` (that getter itself is untouched — NPC reaction chance is unaffected); persisted via new `SaveData.resolvedHiddenFindSpotIds`/`badges` fields; Standing + Badges added to the existing Character screen — grave digging, cemetery/stoneCircle/monolith finds, badge unlocks and the Character screen UI need browser verification |
