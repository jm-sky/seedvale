...
| `2026-08-17--149--shader-program-first-use-hitch.md` | First-use shader/program hitch przy streamingu chunków (frame max / liczba hitchy, nie średni FPS); Phase 0 (dev/benchmark-only `renderer.info.programs`/material census + chunk-attach + mirror/postprocess stage timing, `src/perf/programCensus.ts`) zaimplementowane i technicznie zweryfikowane; agent-browser smoke test już pokazał programDelta>0 skorelowane z wieluset-ms `durationMs` na mirror/postprocess stage'ach, ale to headless/SwiftShader — nie real-GPU dowód; real-GPU `stream` benchmark w Cursor jeszcze nie wykonany; Phase 1 (`compileAsync` tylko w kontrolowanym loading window) niezaczęte (patrz [implementation notes](./2026-08-17--149--shader-program-first-use-hitch-implementation-notes.md)) | 🔴 | M/L | — |
