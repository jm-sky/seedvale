...
| `2026-08-21--181--natural-mountains-and-rivers.md` | Etap 1 (mountain tuning) + Etap 2–3 (pure D8 drainage prototype, `src/terrain/hydrology.ts`) + Etap 4–6 (river network as fixed 256m tiles with a halo, `src/terrain/riverNetwork.ts`/`riverTileCache.ts`, chunk-boundary-continuous ribbon geometry + a distinct lightweight water material reusing `waterMaterial.ts`'s day/night uniforms unmodified, `src/world/riverGeometry.ts`/`riverWaterMaterial.ts`/`createRiverWater.ts`, wired into `chunkManager.ts`) implemented — rivers now actually render in the world. Techniczna weryfikacja zielona (tsc/lint/build/test, 1449 testów) — see plan's "Implementation summary". Etap 7 (meanders, waterfalls, full shader/rendering parity, worker offload) explicitly deferred; bez testu w przeglądarce (user verifies manually) | 🔴 | M | - |
...
| File | Summary | Pri | Effort | Depends |
|------|---------|-----|--------|---------|
| `2026-08-20--179--animal-attack-and-npc-defense.md` | `frenzied` runtime `AnimalAgent` state (`setFrenzyWolf()` DevTools command, `window.seedvale.debug.setFrenzyWolf()` — pure deterministic `pickNearestEligibleWolf()` nearest-wolf/nearest-village selection, no `Math.random()`) reuses the existing predator-human decision unchanged (`provoked: provokedTimer > 0 \|\| frenzied`) instead of a new "reduced fear" branch, plus a strategic village position (not a live `Settlement` ref) driving movement until in range; frenzied wolf → NPC targeting extends `AnimalAgent.update()` with a bounded/local NPC candidate list (loaded settlements' NPCs, never a world scan) only once the player isn't the active threat; `AnimalAgent.isThreateningHuman()` exposes the existing throttled attack/flee decision as a small signal; new `src/ai/npcAnimalThreat.ts` (`senseImmediateAnimalThreat`/`decideAnimalThreatResponse`, same `pickHighestScore` shape as `predatorHumanDecision.ts`) is the NPC-side `ImmediateAnimalThreat` pressure → defend/flee decision, wired into `NpcAgent.update()` ahead of its phase switch so it fires before damage and outranks the player look-at-me pause but never interrupts `combat`/sleep; `defend` calls the existing 177 `beginCombat()` via `fauna/faunaCombat.ts`'s `combatTargetForAnimal()`, `flee` reuses the existing `wander` phase; animal → NPC damage reuses 177's `applyIncomingCombatDamage()`; no new combat/threat-manager/flee system — see [implementation notes](./2026-08-20--179--animal-attack-and-npc-defense-implementation-notes.md); techniczna weryfikacja zielona (tsc/lint/build/test, 1443 testów), bez testu w przeglądarce | 🔴 | M | ~~177~~ |
| `2026-08-21--184--item-capability-abstraction.md` | Deklaratywne `ItemCatalogEntry.capabilities` (`wood_chopping`, `meat_harvesting`, `branch_trimming`, `soil_digging`, `rock_mining`, `fire_starting`, `fishing`) jako jedyne źródło prawdy dla bramek narzędziowych; `hasItemCapability()` + `Inventory.hasCapability()/findWithCapability()`; `HELD_TOOL_KINDS`/`CAPABILITY_KINDS` wyprowadzane z katalogu, `isChopTool`/`isHarvestKnife`/`WELL_STAGE_TOOL` usunięte; `melee`/`ranged`/`defense` i `WeaponMaintenanceKind` celowo bez zmian — [implementation notes](./2026-08-21--184--item-capability-abstraction-implementation-notes.md); techniczna weryfikacja zielona (tsc/lint/build/test, 1428 testów), bez testu w przeglądarce | 🟡 | M | - |
...
| `2026-08-21--183--slope-movement-constraint.md` | Wspólny `terrain/slopeConstraint.ts` (czysty finite-difference slope probe + smoothstep falloff 35°–55°) skaluje/blokuje tylko **uphill** składową per-frame ruchu; ruch w poprzek/w dół bez zmian. Wpięty osobno w `PlayerController.update()`, `NpcAgent.steerTo()`, `AnimalAgent.steerToward()` — trzy niezależne pętle ruchu dzielą jedną funkcję matematyczną, bez navmesha/nowej fizyki, bez zmian w `world/collision.ts`; techniczna weryfikacja zielona (tsc/lint/build/test, 1393 testów), bez testu w przeglądarce | 🟡 | S | - |
| `2026-08-20--177--npc-combat.md` | Melee **and** ranged: `combat/meleeAttack.ts` + `combat/rangedLifecycle.ts` (neutral windUp→hitWindow→recovery / draw→release→recovery timers, extracted from `playerMelee.ts`/`playerRanged.ts`, which now wrap them) + `combat/combatIntent.ts` (`CombatTargetHandle`/`CombatIntent{mode:'melee'\|'ranged'}`, small data-only seam) + `src/ai/npcCombat.ts` (weapon/ammo/defense resolution from `NpcAgent.carried`, hit application for both modes) + `NpcAgent` gained a `combat` `Phase`/`beginCombat()`/`cancelCombat()`/death (`die()` — first thing that can actually kill an NPC); ranged reuses plan 162's `RangedConfig`/`combat/projectile.ts`/`combat/rangedAttack.ts` unchanged — an NPC owns at most one in-flight `Projectile` on itself instead of a shared world registry (no `NpcCombatManager`/global projectile pool); `AnimalAgent.takeDamage()` `source` widened to `'npc'`, `faunaCombat.ts` gained `combatTargetForAnimal()`. Animal-defense (179) is now the first live caller of `beginCombat()` (defend decision); Hunter/bandit deciders remain future callers — see plan's "Implementation summary" §20; techniczna weryfikacja zielona (tsc/lint/build/test, 1374 testów), bez testu w przeglądarce (no test constructs a full `NpcAgent`, matching the rest of this suite) | 🔴 | M | ~~150~~ ~~162~~ |
