...
| `2026-08-17--145--shadow-budget-optimization.md` | Analiza + plan shadow rendering: pull-based fail-open dirty/budget shadow map update (`renderer.shadowMap.needsUpdate` dziś bezwarunkowe co klatkę, `gameLoop.ts`) reużywający `NPC_SHADOW_DISTANCE`/`FAUNA_SHADOW_DISTANCE`, + próg rozmiaru dla proceduralnych item fallbacków (`items.ts`'s `createItemMesh`) analogiczny do istniejącego `SMALL_MESH_SHADOW_THRESHOLD`; potwierdza że NPC/fauna distance-based shadow filtering i terrain frustum culling już istnieją (nic do zrobienia); zero implementacji kodu w tej sesji — czysta analiza + plan | 🟡 | M | — |
