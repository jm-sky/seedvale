...
Combat & weapons
  ...
  (150 combat mode) + (155) → (162 bows/ranged, verification needed) → (177 NPC combat melee+ranged, verification needed — no live AI trigger yet) → 179 animal attack & NPC defense
...
| `2026-08-16--127--player-built-well.md` | Fizyczna studnia budowana przez gracza — `world/playerWell.ts` (stan/koszty/aktywna praca) + `world/createPlayerWells.ts` (lifecycle, collider przez istniejący `ColliderRegistry`) + `world/playerWellProp.ts` (pit/well proceduralne, `roof` = istniejący `createWell()`); Quick Actions "Zbuduj studnię" (łopata, niezużywana) rozpoczyna etap `pit`; nowy `Interactable{kind:'playerWell'}` obsługuje postęp/`[E]` uruchamia sesję aktywnej pracy (busy-channel bout, `workOnWell`) z kosztem kamień/gałąź pobieranym atomowo przy starcie etapu; **rewizja 2026-08-21**: etapy kończą się wyłącznie od `workProgress` (godziny aktywnej pracy gracza), nigdy od upływu czasu świata — usunięto `stageStartedAt`; ukończona studnia (`roof`, praca ukończona) staje się zwykłym `{kind:'well'}` — ten sam mechanizm picia/napełniania co studnia osady; NPC water-fetch (`NpcAgent.resolveWaterWellTarget`) woli bliższą ukończoną studnię gracza od studni osady (bez `if (playerBuiltWell)`, bez kolejki); `SaveData` v24 (`playerWells`, `workProgress` zamiast `stageStartedAt`) — patrz "Implementation summary"/"Revision" planu; techniczna weryfikacja zielona (tsc/lint/build/test, 1408 testów), bez testu w przeglądarce | 🟡 | M | ~~122~~ |
