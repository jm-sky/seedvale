...
| Plan | Sprawdź |
|------|---------|
| `world-terrain-038-shadow-caster-content-budget-v2.md` | Po `storageGoods` no-shadow: `?benchmark=settlement-heavy` → porównaj Isolation `full` vs `no-shadows`, top-level `Shadow casters` (settlement draws), sekcję `Settlement shadow casters` (`other` powinien spaść; `storageGoods` ≈ 0 w shadow). Wizualnie: jedzenie na skrzyniach bez cienia OK; budynki/płoty/NPC bez regresji. |
