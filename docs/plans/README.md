...
| `world-terrain-038-shadow-caster-content-budget-v2.md` | Po census effective-visibility + well split: `?benchmark=settlement-heavy`. Settlement shadow: niższe `storageWood*` (ukryte `Pile_*` nie liczone); nowe `landmarkWellCentral` / `Household` / `Pasture` (suma ≈ N×5 meshes GLB). Isolation `full` vs `no-shadows`. Bez production cutów studni. |
