...
| Plan | Sprawdź |
|------|---------|
| `world-terrain-038-shadow-caster-content-budget-v2.md` | Gate A follow-up: `?benchmark=settlement-heavy` → skopiuj pełną sekcję `Settlement shadow casters (one-pass estimate):` (houseStatic/houseInteractive/fence/storage/workplace/landmark/fireLight/decor/other) oraz top-level `Shadow casters` + Isolation `full` / `no-shadows`. Bez sub-censusu nie wybieramy produkcji content rule. |
