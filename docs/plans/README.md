...
| `2026-08-20--172--modular-quaternius-player-character.md` | Player model swapped from `Adventurer.glb` to Quaternius Universal Base Characters + Modular Character Outfits (male + Peasant); new `characterConfig.ts`/`loadPlayerCharacter.ts` (outfit shares base skeleton, no dup bones), animation clips sourced from Universal Animation Library (base pack ships none) trimmed+compressed into repo; female sex left unwired (raw source assets only) — architecture ready, no `PlayerController` rewrite needed; techniczna weryfikacja zielona (tsc/lint/build/test, 1172 testów), bez testu w przeglądarce (patrz plan's "Implementation summary") | 🟡 | M | - |
