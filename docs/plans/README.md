...
| `2026-08-17--143--cross-chunk-vegetation-batching.md` | Batchowanie roślinności/środowiska przez granice chunków (region N×N=3, rebuild-on-change, nowy `vegetationRegionBatcher.ts`, bez zmian w `instancedProps.ts`/osadzie); techniczna weryfikacja zielona (tsc/build/test 987 testów) + `?benchmark=stream` przed/po w przeglądarce (agent-browser): vegetation/environment draw calls −18%/−21%, brak wzrostu trójkątów, brak hitchy, FPS min 38→51; chop-near-boundary tylko unit-testem, bez live browser (patrz [implementation notes](./2026-08-17--143--cross-chunk-vegetation-batching-implementation-notes.md)) | 🔍 | L | — |
...
| `2026-08-17--144--water-reflection-gpu-optimization.md` | Redukcja kosztu współdzielonego 128² water mirror (uwaga: ID `144` koliduje z `2026-08-17--144--npc-player-food-drink-help.md`). Stage S (reflection visibility budget) zaimplementowany: nowa `REFLECTION_DISTANT_LAYER` (odróżniona od `REFLECTION_SKIPPED_LAYER`, bo ta treść nadal rzuca cień — main + shadow camera ją włączają, mirror camera nie), `chunkManager.ts`'s `syncReflectionForRecord` wyłącza z mirror pass mesh/vegetationExtras/environment tylko najdalszego pierścienia streamingu (`reflectionVisibleRadius = loadRadius − 1`), `vegetationRegionBatcher.ts`'s nowy `syncReflectionVisibility` (ta sama "nearest member wins" reguła co `syncLod`) robi to samo dla batchowanej roślinności/rocks — bez nowego systemu streamingu/visibility. Stage M/L nieruszone (gate'owane benchmarkiem, który nie został odpalony w tej sesji). Techniczna weryfikacja zielona (tsc/test 1000/build); brak benchmarku `?benchmark=water`/`?benchmark=stream` i brak testu w przeglądarce (patrz [implementation notes](./2026-08-17--144--water-reflection-gpu-optimization-implementation-notes.md)) | 🟡 | M | ~~143~~ |
...
| File | Summary | Pri | Effort | Depends |
|------|---------|-----|--------|---------|
| `2026-08-17--148--grass-gpu-performance.md` | Grass geometry LOD (S): `InstancedMesh.geometry` per species bucket (`tri`/`grain`/`herb`) now swaps between `near`/`mid`/`far` fin-cluster variants (`src/terrain/grass.ts` — same fin arrangement, fewer segments per tier, `grain`'s far tier also drops its leaf fin) as chunks cross distance bands, orthogonal to the existing instance-count `setLodFraction`; tiers anchored to `densityLodFraction`'s own near-field breakpoint via new `grassGeometryLodTier()` (`distanceLod.ts`) so geometry LOD doesn't fight density LOD; per-tier geometry built lazily and cached, instanced attributes shared by reference across tiers (no GPU buffer duplication); filler bucket untouched. Techniczna weryfikacja zielona (tsc/build/test, 1000 testów, +3 nowe); `?benchmark=current` przed/po (agent-browser, headless, jeden run na stan): grass triangles −47.0% (8.54M→4.53M), scena total −25.1% (identyczna delta co grass — zmiana w pełni izolowana), draw calls bez zmian; **FPS/RENDER bez poprawy** (frame p95 +26%, min FPS 44→32, RENDER płaskie) — sygnał że w tym scenariuszu grass vertex count nie jest głównym bottleneckiem, nie potwierdzone na realnym GPU ani powtórzonymi runami; M (density tuning)/M (shader simplification) świadomie wstrzymane do realnego pomiaru, L (billboard) odłożone jak w planie — [implementation notes](./2026-08-17--148--grass-gpu-performance-implementation-notes.md) | 🟡 | L | — |
...
| `2026-08-17--141--animal-traps.md` | Pułapki na zwierzęta jako placed world object obok `PlacedTents`/`PlacedFires` (`world/createPlacedTraps.ts` w `WorldBundle`), nie nowy manager: 2 rodzaje (`simple`/`good`) w `TRAP_DEFS`, stany `placed → active → placed/broken`, snapshot `PlayerSkills.traps.value` przy uzbrojeniu (pułapka nie zna gracza), throttlowany (0.5 s) proximity check po liście agentów fauny zamiast per-frame skanu, leniwe zużycie pogodowe po zakończonych cyklach `computeWeather` (tylko uzbrojone), cooldown per `(pułapka, animalId)` runtime-only; złapanie → `AnimalAgent.takeDamage()`/`collapse()` + gatunkowe mięso jako world item (wydzielone `fauna/animalMeat.ts`) i tusza oznaczona jako oprawiona; skill `traps` na wspólnej krzywej, XP tylko za capture; zastawianie z Quick Actions przez wspólne `evaluateGroundPlacement()`; save v16 (`placedTraps` + `skills.traps`); model 3D proceduralny — GLB jako [MODELS.md M40](../assets/MODELS.md). Techniczna weryfikacja zielona (tsc/build/test 976); brak testu w przeglądarce — [implementation notes](./2026-08-17--141--animal-traps-implementation-notes.md) | 🟡 | M | ~~128~~ |
| `2026-08-17--142--mobile-combat-target-acquisition.md` | Melee na dotyku bez osobnego systemu combat: nowy `CombatAimMode` (`'pointer'` / `'touch'`) wybiera stożek akwizycji (`COMBAT_TARGET_CONE_DOT` jako tablica per-tryb — `pointer` zostaje na `cos(45°)`, `touch` 0.3/~145°) w istniejącym `buildCombatTarget()`/`pickCombatTarget()`; auto-facing = commit yaw ataku na cel (`yawToward()` w `playerMelee.ts`, ustawiane po `gapClose()`, konsumowane raz przez `resolveMeleeHits`) — konieczne, bo hit test liczy się z yaw kamery, a nie z rotacji mesha, którą `faceToward()` już zmieniał; kamera/ruch/lock-on nietknięte, desktop bit-w-bit bez zmian (`attackYaw` zawsze `null`). Bez zmian damage/staminy/timingów/zasięgu. Techniczna weryfikacja zielona (tsc/build/test 950, eslint czysty na `src/`); brak testu w przeglądarce — [implementation notes](./2026-08-17--142--mobile-combat-target-acquisition-implementation-notes.md) | 🟡 | S | ~~123~~ ~~124~~ |
