...
| [2026-08-07--003--day-night-clock.md](./2026-08-07--003--day-night-clock.md) | Zegar dnia/nocy + time multiplier | `in progress` 🔄 (3/5 — brak: NPC sen zależny od pory, persystencja `timeOfDay` w save) |
...
| [2026-08-07--016--ambient-world-audio.md](./2026-08-07--016--ambient-world-audio.md) | Ambient audio zależne od obszaru (świerszcze/ptaki dzień-noc, szum fal blisko oceanu) | `in progress` 🔄 (2/5 — fundament audio + warstwa dzień/noc done; wiring assetów lasu/oceanu, sampler obszaru, mixer runtime nie zaimplementowane) |
...
| [2026-08-07--020--npc-2-daily-routine-and-place.md](./2026-08-07--020--npc-2-daily-routine-and-place.md) | NPC Place system, przycięty do minimum: formalizacja `home` (już działa de facto); per-rola workplace/schedule odłożone | `in progress` 🔄 (~15% pełnego pierwotnego zakresu — v1: formalizacja `home` jako `Place` **done**; `PlaceType` ograniczony do `'home'`, `workplace`/`food`/`social` **nie zaimplementowane**; `Schedule Template` **nie zaimplementowany**; `role` istnieje tylko jako dane (`NpcAgent.ts:233`), zero wpływu na zachowanie; generyczny FSM `goTo`/`execute` **nie zaimplementowany** — pełna lista w pliku planu, sekcja „Odłożone") |
...
| [2026-08-08--038--campfire-lighting.md](./2026-08-08--038--campfire-lighting.md) | Zapalanie ognisk gałęziami (paliwo/czas palenia/dokładanie) + wolnostojące ogniska budowane przez gracza (2x gałąź + 2x kamień, menu pauzy) — zaimplementowane; 50% szans na już zapalone ognisko wioski w nocy — nadal `todo` | `verification needed` 🔍 (3/4 — punkty 1-3 done, punkt 4 nocne 50% szans `todo`) |
