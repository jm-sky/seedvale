...
| Plan | Sprawdź |
|------|---------|
| `world-terrain-038-shadow-caster-content-budget-v2.md` | Po exterior-lamp no-shadow + wood/landmark split: `?benchmark=settlement-heavy` → Isolation `full` vs `no-shadows` (nie porównuj FPS do 025 — isolation probes były uszkodzone). Settlement shadow: `fireHouseExteriorLamp` ≈ 0; nowe wiersze `storageWoodSettlementPrimary/Overflow/Secondary`, `storageWoodHousehold`, `landmarkWell/Garden/Field/NoticeBoard`. Wizualnie: exterior lampy bez cienia OK; interior/torches/campfire/budynki bez regresji. |
