...
- [ ] 2026-08-17 — kod diagnostyczny AO w `src/render/createPostProcessing.ts` (globalny `__aoDiag`, N8AO przypięte ON, `applyFrameBudget` tylko loguje zamiast stosować `syncAoPass()`) nadal aktywny na `main`: auto-budget z planu 113 faktycznie nie działa i każdy pomiar AO mierzy pass przypięty na stałe. Do usunięcia po zamknięciu wątku ~20 Hz tremble (research `2026-08-17--017--threejs-rendering-audit.md` §4 F5, `2026-08-17--015` §7)
