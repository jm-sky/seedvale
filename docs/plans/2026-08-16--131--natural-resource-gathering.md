...
**Created:** 2026-08-16
**Status:** `verification needed` 🔍 — implemented per the narrower contracts in the [implementation notes](./2026-08-16--131--natural-resource-gathering-implementation-notes.md): wood's existing `chop → deposit` now only credits the household/economy when `harvestWorldTreeFully` actually succeeds (was unconditional — a contested tree could mint wood from a failed harvest); NPC ore gathering is new (`miner`'s `work` block tries a real `ResourceDeposits` extraction via injected hooks before falling back to the pre-131 idle stand), reusing the player's `mine()`/`yieldForOre()` and a small NPC-carried `Inventory` (existing class, now a generic carrier) for the atomic "check carry room → extract → add" contract; ore is new settlement-level `EconomicKind` stock (`iron`/`coal`/`gold`), not `Household` (still `food`/`wood` only — `HouseholdResourceKind` decoupled from `EconomicKind` so it can't silently inherit new kinds); `ResourceDeposits.update()` now also keeps deposits loaded near active settlement centers, not just the player, so mining works without the player nearby. Technical verification (`tsc`/`test`/`build`) green; no browser verification yet. Food gathering was audited and left unchanged — its existing garden→household path already satisfies the plan's food acceptance criteria as-is. `fish`/`herbs`/`clay`/`salt`/`resin`/`fertile_soil` remain deferred (no real world source/action), per plan scope.
