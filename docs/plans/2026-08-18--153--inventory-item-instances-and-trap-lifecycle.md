...
jest tożsamością tego samego egzemplarza przed i po postawieniu.
...
Selekcja powinna być funkcją domenową, a nie logiką komponentu UI, np. logicznie:

```text
selectInstancesForAutoSell(...)
```

Nie kodować tego jako wyjątku dla pułapek.
...
Nie migrować automatycznie:

- wood;
- shell;
- coin;
- food;
- innych obecnych stackable items.

Muszą nadal działać przez istniejące count-based API.
...
- [ ] Broken może zostać zebrana i przechowana jako instance.
- [ ] Broken można sprzedać za bardzo niską cenę.
- [ ] Cena używanego itemu uwzględnia condition i centralny bazowy zakres 10–25% discount.
