...
> **superseded by the revision below** — this implementation summary describes the original elapsed-world-time timer model, which turned out to be wrong (see the revision's "Why" for the bug report). Kept as history, not as the current implementation.

## Revision (2026-08-21) — active-work model

**Bug report that triggered this revision:** the 2026-08-21 implementation above made every stage complete purely from elapsed `dayNight.elapsedDays` (`stageStartedAt` + `WELL_STAGE_DURATION_DAYS`), evaluated lazily whenever the record was looked at. World time advances every frame regardless of the player (`gameLoop.ts` calls `tickDayNight` unconditionally) — so a stage finished whether or not the player ever came back, did anything, or even kept the game open. That is not "the player built a well," it is "the player waited." The fix: construction is now **active player work**. A stage's clock only ever advances while the player is actually engaged in a work session; walking away, closing the game, or letting world time pass on its own does nothing.

**New model:**

- `PlayerWellRecord.stageStartedAt` is deleted. Replaced by `workProgress: number` — hours of *active* work completed toward the current stage's requirement (`WELL_STAGE_WORK_HOURS: { pit: 2, well: 2, roof: 1 }`, preserving the original plan's `pit : well : roof = 1 : 1 : 0.5` proportions, now expressed as active-work hours instead of elapsed-time days — a chosen value, not derived from other existing gameplay, since none was prescribed). `isWellStageWorkComplete`/`isWellCompleted`/`activeWellStage` are pure functions of the record alone — no `nowDays`/elapsed-time argument exists anywhere in this model any more.
- Each stage has an explicit, data-driven contract: `WELL_STAGE_TOOL` (tool required, never consumed — `pit: 'shovel'`, `well`/`roof`: `null`, since no existing item fits "assemble stone/wood into a well body or roof" and inventing one would be an artificial requirement) + `WELL_STAGE_COST` (unchanged: `pit` free, `well` 6 stone + 3 branch, `roof` 4 branch) + `WELL_STAGE_WORK_HOURS`.
- Materials are consumed atomically the moment a stage's *first* work session begins (validate tool + all materials → nothing missing → consume once), never before (nothing is spent if validation fails) and never again on later resume sessions of the same stage.
- Active work reuses the existing `busyAction` channel (`src/app/busyAction.ts`) — the codebase's one canonical timed-player-action primitive (also used by digging/chopping/mining/harvesting/igniting/cooking/tent-setup) — instead of any new system. A full stage's work is reached over several short, repeated "bouts" (`WELL_WORK_SESSION_SEC = 4`, added to `busyChannelDurations.test.ts`'s regression guard) rather than one long frozen channel; a prior regression in this same codebase turned 120–300s "channels" into something that "read as a freeze," which a stage's full ~2h-equivalent of work would reintroduce as a single channel. Each `[E]` press runs one bout; the *measured* world-time delta over the bout (not an assumed value) is credited to `workProgress` on both natural completion and on Escape-cancellation (`onCancel`) alike — so an interruption keeps exactly the work actually done, never rolling back stage or materials, and resumes from there on the next press.
- Reaching a stage's `workRequired` makes the next stage's `[E]` prompt (`activeWellStage`) available immediately — `app/interactables.ts` re-derives it fresh every frame, so there is no extra waiting and no separate "advance" step.
- `SaveData` bumped v23 → v24: `SavePlayerWell.stageStartedAt` → `workProgress`. This is a genuine schema change (a field's *meaning* changed, not just an addition), so `toV24` is a real migration — pre-v24 in-progress wells keep their `stage` but reset `workProgress` to 0, since there is no way to recover "how much was *active* work" from a value that was only ever an elapsed-time timestamp. This is an accepted one-time migration cost, not silently auto-completing a stage or fabricating progress.

**Files touched:** `world/playerWell.ts` (full rewrite of the stage/timer logic), `world/createPlayerWells.ts` (`advanceStage` → `addWork` + `transitionTo`; dropped the now-unneeded `getWorldDays` dependency entirely), `app/worldBundle.ts` (two `createPlayerWells(...)` call sites), `app/actions/placementActions.ts` (`advanceWellStage` → `workOnWell`), `app/interactables.ts`, `app/gameLoop.ts`, `app/createApp.ts` (rename threading), `app/busyChannelDurations.test.ts`, `persistence/saveData.ts` (v24), `persistence/saveData.test.ts`, `persistence/saveSlots.test.ts` (hardcoded `version` assertions), `app/saveState.ts` (`SAVE_VERSION`), `world/playerWell.test.ts` (full rewrite), `world/createPlayerWells.test.ts` (new).

**Unchanged by this revision:** placement (Quick Actions "Zbuduj studnię", shovel required/never consumed, ground-placement validation), `well`/`roof` visuals per stage, the completed-well → `WaterSource` handoff (`createWaterSource('well')`, exact same drink/fill interaction as any well), and NPC discovery (`NpcAgent.resolveWaterWellTarget`/`getNearbyPlayerWell`) — none of plan 127's §9/§10 water-source/NPC integration needed to change.

**Technically verified:** `npx tsc --noEmit`, `pnpm run lint:fix`, `pnpm run build`, `pnpm run test` all pass (1408 tests, including the rewritten `world/playerWell.test.ts`, new `world/createPlayerWells.test.ts`, and updated `persistence/saveData.test.ts`/`saveSlots.test.ts` v24 coverage).

**Browser/manual verification — not performed.** Needs a manual pass: place a well → dig the pit across several `[E]` presses (bouts) → confirm no progress accrues while away/idle or with the game closed → interrupt mid-bout with Escape and confirm the partial progress is preserved on the next press → reach `well`/`roof` and confirm each stage's `[E]` prompt flips to the next stage's verb immediately, with no extra waiting → confirm the completed well still drinks/fills exactly like before → save/load mid-stage and confirm `workProgress` persists and does not grant any progress for time spent with the game closed.
