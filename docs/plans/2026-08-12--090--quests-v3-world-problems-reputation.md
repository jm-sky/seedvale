...
Nie tworzyć osobnych mechanizmów walki, HP/death, zwierząt, landmarków, inventory, interakcji ani dialogów. Quest powinien obserwować stan tych systemów lub reagować na ich zdarzenia.
...
Docelowo relacja może istnieć wobec NPC, Village oraz Faction / group. W pierwszej implementacji należy jednak użyć istniejącego modelu relacji per NPC i nie budować jeszcze pełnego systemu frakcji.

Quest może wymagać minimalnego poziomu relacji z konkretnym NPC. Jeśli stabilna `VillageIdentity` już istnieje w codebase, można przygotować rozszerzenie pod reputację wioski, ale nie należy tworzyć równoległego systemu tylko na potrzeby tego planu.
...
Rozszerzyć `QuestDef` o opcjonalne warunki dostępności, np. minimalny poziom relacji z konkretnym NPC.

Quest niespełniający warunków nie powinien być oferowany NPC. Po osiągnięciu wymaganego poziomu relacji powinien stać się dostępny bez potrzeby tworzenia nowego questa.
...
Dokładne wartości liczbowe pozostawić jako szczegół implementacyjny i skonfigurować w jednym miejscu.

Nie projektować jeszcze rozbudowanej reputacji z dziesiątkami rang, globalną reputacją, frakcjami ani reputacją regionalną. Architektura ma umożliwiać późniejsze rozszerzenie.

## Faza 3 — Reputacja jako efekt questa

Quest może zmieniać relację, np. przez efekt końcowy `relation: +1` lub równoważny model zgodny z istniejącą architekturą.

Efekt powinien korzystać z istniejącego `QuestManager` i jego relation state. Nie tworzyć drugiego mechanizmu zapisywania relacji.
...
Warunek: `required relation: trusted`.
...
Quest nie powinien wykonywać własnej logiki walki. Powinien reagować na istniejący flow:

```text
AnimalAgent → HealthState → damage → death → quest objective completed
```
...
Aby questy dotyczące konkretnego zwierzęcia były możliwe, zwierzę gospodarskie musi posiadać stabilną identity oraz informację o właścicielu / gospodarstwie.

Nie implementować questów typu „znajdź konkretną owcę”, dopóki nie ma stabilnej tożsamości zwierzęcia.

## Faza 7 — Questy związane z landmarkami

Wykorzystać istniejący system proceduralnych landmarków, w tym ruiny i kamienne kręgi.

Quest może wskazywać `landmarkId` lub `landmarkType`.
...
```text
landmark → może istnieć bez questa
quest → może opcjonalnie wskazywać landmark
```
...
Questy związane z drewnem powinny korzystać z istniejącego lifecycle drzewa, a quest nie powinien sam zarządzać tym cyklem.
...
System questów powinien już umożliwiać model:

```text
NPC / world event → hostile group → world problem → quest
```
...
Później mogą dojść inventory, money, world state, village state i NPC state. Nie implementować wszystkich typów od razu.

## Faza 11 — UI i feedback

Quest log powinien pokazywać stan wymaganej reputacji, jeżeli ma to znaczenie, np.:
...
Nie ujawniać koniecznie wszystkich ukrytych questów. Jeżeli design wymaga świadomego komunikowania blokady, NPC może powiedzieć, że nie jest jeszcze gotowy powierzyć sprawy graczowi.

---

# Persystencja

Jeżeli aktualny system questów nadal przechowuje stan wyłącznie w pamięci, nie rozszerzać tego planu o pełną persystencję, chyba że aktualny codebase w międzyczasie już ją wprowadził.

Jeżeli relacja zostanie później zapisywana w save, Quest state i Relation state powinny być traktowane jako część jednego persistent simulation state.
