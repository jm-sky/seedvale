...
- **Program `cacheKey`/`name` still not collected** — review 021 closed the two Phase 0 gating questions without a per-program key list. Phase 1 A still needs that dump before staging prewarm (plan open question 7).
...
Done in [review 021](../reviews/2026-08-18--021--plan-149-phase-0-real-gpu.md) (2026-08-18, 3 cold `stream` runs, Intel Arc 140V). Outcome: bounded ~230 programs, family-first-use + plateau, hitch paid by mirror and/or postprocess; recommend **A** (loading-time prewarm) after a `cacheKey` dump. Do not implement A from the SwiftShader smoke test in §3.

## 7. Recommendation for next step

**Phase 0 closed** — see [review 021](../reviews/2026-08-18--021--plan-149-phase-0-real-gpu.md). Next: read-only dump of `renderer.info.programs` `cacheKey`/`name` from one cold `stream` run, then Phase 1 **A** (loading-window prewarm of that list, including mirror + composer state). Do not revive per-chunk/per-tick/full-scene `compileAsync()`. Consolidation (B) is a Phase 2 candidate for the ~600 material UUIDs, not the first hitch fix.
