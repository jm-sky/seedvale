...
**Implemented, technically verified** (`npx tsc --noEmit`, `npm run lint`, `npm run build`, `npm run test` all pass — 464/464 tests). **Browser/manual verification not yet done** — see the checklist in §15 above; still needs to be run against the live dev server before this plan can move to `done`.
...
- `src/fauna/AnimalAgent.ts` — the bulk of the work:
  ...
  - `findCarcassTarget()` reuses the existing `others`/`health.dead`/corpse-lifetime machinery; added a minimal `foodClaimedBy: AnimalAgent | null` guard (`claimAsFood`/`releaseFoodClaim`) per §8, not a global carcass manager.
...
- `src/fauna/foodWaterTargeting.test.ts` (new) — pure unit tests for `shoreProbeHits`/`forageEdgeScore` per §14's "keep target-selection tests pure where practical".

**Deliberate scope call on §2/§7 (which animals really forage):** rather than special-casing deer/stag, forage was implemented generically for any non-predator (`role !== 'predator'`) — this covers wild prey (deer, stag, rabbit, duck, boar) and, since `sampleForestFactor` is an **optional** constructor parameter (`undefined` for `src/settlement/livestock.ts`'s spawn path, unchanged call site), also domestic livestock (sheep, chicken) via the same code path with distance-only scoring (`suitability` defaults to `0.5` when no sampler is available) instead of habitat-biased scoring. This was explicitly permitted by §2 ("domestic livestock: forage as well, if the existing livestock path already uses `AnimalAgent` hunger/thirst") and keeps the seam small — no threading `sampleForestFactor` through `spawnLivestock()`/settlement creation. Horse/donkey/cow (`role: 'livestock'`) still fall through to the old no-real-source behavior (needs stay elevated, wander bias only) since the plan's role table doesn't cover them and wasn't asked to.
