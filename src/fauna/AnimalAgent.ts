...
import { createSaddlebagsAttachment, disposeSaddlebagsAttachment } from './animalPackPresentation'
import {
  type AnimalTrip,
...
export type AnimalSaveState = {
  ...
  /** Equipped saddlebags pack (plan fauna-039) — omitted means no pack
   *  equipped, the same convention every other optional field here uses.
   *  Capacity is never persisted, only contents — see `hydrateAnimalPack`. */
...
export class AnimalAgent {
  ...
  /** Equipped saddlebags pack (plan fauna-039) — `null` means no pack
   *  equipped, the sole ownership authority (never a second boolean flag).
   *  Only a player-owned animal may ever have one (`canEquipAnimalPack`). */
  private _pack: AnimalPackState | null = null
  /** Attached pack visual (plan fauna-039 §9) — presentation-only, a child
   *  of `mesh`; always kept in sync with `_pack !== null` by
   *  `syncPackVisual()`. Never authoritative — a load failure still leaves
   *  `createItemMesh`'s procedural fallback attached. */
  private packVisual: THREE.Object3D | null = null
  /** This tick's `playerControlPos` from `AnimalUpdateContext` (plan fauna-020). */
  private _tickPlayerControlPos: { x: number, z: number } | null = null
  /** Actor-neutral temporary follow target for a merchant pack-animal
   *  relation (plan settlements-npcs-048) — set externally by
   *  `SettlementsManager` each frame from the assigned merchant's live world
   *  position (`null` when the merchant isn't currently live/assigned).
   *  Never a `NpcAgent` reference, never player semantics, never persisted:
   *  reconstructible from `MerchantJourneyState.packAnimalId` alone. */
  private _merchantFollowTarget: { x: number, z: number } | null = null
  /** Runtime-only merchant-follow hysteresis — separate from lead/owned-
   *  control commitment (plan settlements-npcs-048). */
  private readonly _merchantFollow: FollowHysteresisState = { following: false }
  ...
  dispose(): void {
    ...
    if (this.packVisual) {
      disposeSaddlebagsAttachment(this.packVisual)
      this.packVisual = null
    }
  }
  ...
  /** Derived household home id for legacy call-sites (plan fauna-020). */
  get ownerHouseId(): string | undefined {
    return deriveOwnerHouseId(this._owner)
  }

  getOwner(): AnimalOwner {
    return this._owner
  }
  ...
  isDraftAnimal(): boolean {
    return isDraftDef(this.def) && !this.health.dead
  }

  isLeadAttached(): boolean {
    return this._leadAttached
  }

  /** Temporary lead input from the action layer. Idempotent; does not
   *  mutate `AnimalOwner` or persisted Follow/Stay. */
  ...
  /** This tick's actor-neutral merchant follow target (plan
   *  settlements-npcs-048) — `SettlementsManager` calls this every frame for
   *  a detached individual currently assigned as a merchant's pack animal,
   *  `null` once the merchant isn't live/assigned. Never mutates ownership. */
  setMerchantFollowTarget(pos: { x: number, z: number } | null): void {
    this._merchantFollowTarget = pos
    if (!pos) this._merchantFollow.following = false
  }
  ...
  /** True for any animal whose `def` carries a `pack` config (plan
   *  fauna-039 §2) — species-agnostic, same "presence is capability" shape
   *  as `isMountable()`. Not by itself equip eligibility — see
   *  `canEquipPack()`. */
  hasPackCapability(): boolean {
    return this.def.pack !== undefined
  }
  ...
  /** Live pack `Inventory`, or `null` if none is equipped — re-resolved by
   *  the app-layer container-transfer session on every mutation, never held
   *  across an open screen. */
  getPackContents(): Inventory | null {
    return this._pack?.contents ?? null
  }

  /** Equip eligibility (plan fauna-039 §6) — does not check the player
   *  actually holds a `saddlebags` item; the caller must do that itself. */
  canEquipPack(): boolean {
    return canEquipAnimalPack(this.def, {
      alive: !this.health.dead,
      playerOwned: this.isPlayerOwned(),
      hasPack: this._pack !== null,
    })
  }

  /** Attaches a fresh, empty pack + its visual (plan fauna-039 §7). The
   *  caller is responsible for removing exactly one `saddlebags` from the
   *  player's own inventory — first checking `canEquipPack()`, and rolling
   *  the item back if this returns `false` (should only happen if state
   *  changed between the check and the call). */
  equipPack(): boolean {
    if (!this.canEquipPack()) return false
    const pack = createAnimalPack(this.def)
  ...
  /** Unequip eligibility (plan fauna-039 §14) — only an empty pack. */
  canUnequipPack(): boolean {
    return canUnequipAnimalPack(this._pack)
  }

  /** Removes an equipped, empty pack + its visual, returning the (empty)
   *  snapshot so the caller can hand exactly one `saddlebags` back to the
   *  player. `null` if there's nothing legally to unequip — re-checks
   *  emptiness itself, so a caller doesn't need to trust a stale read. */
  unequipPack(): AnimalPackSnapshot | null {
    if (!this.canUnequipPack() || !this._pack) return null
    const snapshot = detachAnimalPack(this._pack)
    this._pack = null
    this.syncPackVisual()
    return snapshot
  }

  /** Death-handoff seam (plan fauna-039 §20/§29) — unconditionally detaches
   *  whatever pack this animal currently carries (equipped or not,
   *  empty or not) and clears it, regardless of `canUnequipPack()`. Only
   *  the composition layer (never `AnimalAgent` itself) may call this: it
   *  owns handing the returned snapshot's contents to a persistent ground
   *  container. `null` if there's no pack to hand off. */
  takePackForHandoff(): AnimalPackSnapshot | null {
  ...
  /** Keeps the attached visual in sync with `_pack` — idempotent, safe to
   *  call any number of times (hydrate/equip/unequip/handoff all funnel
   *  through this single seam so stream/load can never duplicate or leak
   *  the mesh, plan fauna-039 §9). */
  private syncPackVisual(): void {
    if (this._pack && !this.packVisual) {
      this.packVisual = createSaddlebagsAttachment(this.def.kind)
      this.mesh.add(this.packVisual)
    } else if (!this._pack && this.packVisual) {
      disposeSaddlebagsAttachment(this.packVisual)
      this.packVisual = null
    }
  }
  ...
  setMounted(mounted: boolean): void {
    ...
    if (mounted) this.setLeadAttached(false)
    if (!mounted) {
      this.moving = false
  ...
  snapshot(): AnimalSaveState {
    return {
      ...
      ...(this.injuryRecovery.injuryRecoveryUpdatedAtDays != null
        ...
        : {}),
      pack: snapshotAnimalPack(this._pack),
    }
  }
  ...
  hydrate(state: AnimalSaveState): void {
    ...
    this._pack = hydrateAnimalPack(this.def, state.pack)
    this.syncPackVisual()
  }
  ...
  private updatePrey(
    ...
    if (this.pursueMerchantFollow(dt)) return
    if (this.pursueRoutineRest(dt)) return
    this.setIntent('wander')
    this.wander(dt)
  }

  /** Temporary lead movement after needs/threat (plan fauna-007) — same
   *  normal-behaviour seam as owned Follow, never a second follow AI. */
  private pursueLead(dt: number): boolean {
    const movement = resolveLeadMovement(
      this._leadAttached,
      this._leadFollow,
      this.mesh.position,
  ...
  /** Player-owned Follow movement after needs/threat (plan fauna-020). */
  private pursueOwnedControl(dt: number): boolean {
    const movement = resolveOwnedControlMovement(
      this._control,
      this.isPlayerOwned(),
      this.mesh.position,
      this._tickPlayerControlPos,
  ...
  /** Merchant pack-animal follow after needs/threat (plan settlements-npcs-048
   *  §12/§13/§14) — same normal-behaviour seam as `pursueLead`/
   *  `pursueOwnedControl`, reusing the shared `followHysteresis` primitive
   *  with the same tight lead trailing band (`animalLead.ts`), never a
   *  second follow/formation system. Placed below `pursueNeeds`/
   *  `pursueAttraction`/`pursueOwnedControl` so urgent needs and other
   *  higher-priority survival behaviour keep interrupting it; it resumes on
   *  its own once those clear, purely from `_merchantFollowTarget` being
   *  set again next tick. */
  private pursueMerchantFollow(dt: number): boolean {
    if (!this._merchantFollowTarget || this.health.dead || this.mounted) return false
    const movement = resolveFollowHysteresis(
      this._merchantFollow,
      this.mesh.position,
      this._merchantFollowTarget,
      leadStartDistance(this.def),
      leadStopDistance(this.def),
    )
    if (movement.kind !== 'follow') return false
    this.cancelSourceTarget()
    this.setIntent('wander', { x: movement.x, z: movement.z })
    this.sourceDest.set(movement.x, 0, movement.z)
    this.steerToward(this.sourceDest, this.walkSpeedNow(), dt)
