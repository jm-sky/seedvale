...
describe('cave surface shader contract', () => {
  it('perturbs the geometric view normal instead of replacing it through tbn or vWorldNormal', () => {
    ...
    expect(fragmentShader.match(/caveWetnessMask\(\s*vWorldPos,\s*geoWorld\s*\)/g)?.length).toBe(1)
    expect(fragmentShader).toContain('wetMask * uCaveWetDarkening')
    expect(fragmentShader).toContain('mix( uCaveDryRoughness, uCaveWetRoughness, wetMask )')
    ...
    expect(fragmentShader.match(/geoWorld = caveViewToWorldDir\(\s*normalize\(\s*vNormal\s*\)\s*\)/g)?.length).toBe(1)
