...
import { isSaveDataV23, loadSaveData, type SaveConfig, type SaveDataV10, type SaveDataV11 } from './saveData'
...
describe('loadSaveData v11 map discovery', () => {
  ...
  it('keeps discovered cells from a v11 save', () => {
    ...
    expect(isSaveDataV23(loaded)).toBe(true)
...
describe('loadSaveData v23 player-built wells (plan 127)', () => {
  const v22 = {
    ...v10,
    version: 22,
    map: { discoveredCells: [] },
  ...
  it('round-trips player-built wells from a native v23 save', () => {
    const playerWells = [
      { id: 'well:1', x: 5, z: -3, yaw: 0.4, stage: 'well' as const, stageStartedAt: 3.5 },
    ]
    const loaded = loadSaveData({ ...v22, version: 23, playerWells })
    expect(isSaveDataV23(loaded)).toBe(true)
    expect(loaded?.playerWells).toEqual(playerWells)
  })

  it('rejects malformed v23 player-well fields', () => {
    const base = { ...v22, version: 23 }
    expect(loadSaveData({ ...base, playerWells: [{ id: 'w', x: 0, z: 0, yaw: 0, stage: 'roofed', stageStartedAt: 0 }] })).toBeNull()
