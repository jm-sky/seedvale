...
export const ITEM_CATALOG: Record<ItemKind, ItemCatalogEntry> = {
  ...
  herb: {
    ...
    spawn: 'world_chunk',
    modelUrl: null,
    notes: 'Plan items-player-043 — rare medicinal/quest flora (`terrain/chunkItems.ts`), renewable after 7 game days. Stronger poisoning treatment than mint. Merchant stock at 12 coins as reachability fallback for `ziola-dla-anny`.',
    consumable: { need: 'health', relief: 8 },
    conditionTreatment: { kind: 'poisoning', severityReduction: 24 },
  },
  mint: {
    kind: 'mint',
    label: 'mięta',
    ...
    spawn: 'world_chunk',
    modelUrl: null,
    notes: 'Plan items-player-043 — common medicinal forage; V1 treats poisoning. Renewable after 1.5 game days. Not Kupiec stock.',
    consumable: { need: 'health', relief: 2 },
    conditionTreatment: { kind: 'poisoning', severityReduction: 12 },
  },
  yarrow: {
    kind: 'yarrow',
    label: 'krwawnik',
    ...
    spawn: 'world_chunk',
    modelUrl: null,
    notes: 'Plan items-player-043 — common/moderate wound herb. Catalog `injuryTreatment` for NPC consumers; weaker than bandage. Renewable after 2 game days. Not Kupiec stock.',
    consumable: { need: 'health', relief: 5 },
    injuryTreatment: { immediateHp: 12, maxSeverity: 'minor' },
  },
  bandage: {
    kind: 'bandage',
    label: 'opatrunek',
  ...
  dagger: {
    ...
    holdable: true,
    capabilities: ['branch_trimming', 'meat_harvesting'],
    melee: { damage: 14, range: 1.7, arcDot: 0.58, windUp: 0.11, hitWindow: 0.08, recovery: 0.17, staminaCost: 4 },
    defense: { canBlock: true, baseBlockChance: 0.14, partialReduction: 0.38 },
    spawn: 'none',
    modelUrl: null,
    notes: 'Plan items-player-047 — Kupiec stock. Better knife, still harvests corpses; distinctly weaker than damascus_knife.',
  },
  hatchet: {
    kind: 'hatchet',
    label: 'toporek',
    holdable: true,
    melee: { damage: 18, range: 1.85, arcDot: 0.42, windUp: 0.2, hitWindow: 0.1, recovery: 0.28, staminaCost: 7 },
    defense: { canBlock: true, baseBlockChance: 0.16, partialReduction: 0.4 },
    spawn: 'none',
    modelUrl: null,
    notes: 'Plan items-player-047 — Kupiec stock. Compact personal weapon; no wood_chopping capability (not a working tool like axe/battle_axe).',
  },
  leather_armor: {
    kind: 'leather_armor',
  ...
  leather_pauldron: {
    ...
    spawn: 'none',
    modelUrl: null,
    notes: 'Plan items-player-039 — Kupiec stock. Wearable arms armor; runtime UBC pauldron overlay, not a ground GLB.',
    armor: {
      slot: 'arms',
  ...
  ranger_pauldron: {
    ...
    spawn: 'none',
    modelUrl: null,
    notes: 'Plan items-player-039 — Kupiec stock. Wearable arms armor; light restriction vs full body armor.',
    armor: {
      slot: 'arms',
      damageReduction: 0.05,
      staminaCostMultiplier: 1.02,
      meleeRecoveryMultiplier: 1.02,
      sprintStaminaMultiplier: 1.04,
    },
  },
  knight_pauldron_spike: {
    ...
    spawn: 'none',
    modelUrl: null,
    notes: 'Plan items-player-039 — Kupiec stock. Wearable arms armor; penalties stay well below chainmail.',
    armor: {
      slot: 'arms',
      damageReduction: 0.07,
      staminaCostMultiplier: 1.06,
      meleeRecoveryMultiplier: 1.05,
      movementSpeedMultiplier: 0.99,
      sprintStaminaMultiplier: 1.08,
    },
  },
  knight_pauldron_round: {
    ...
    spawn: 'none',
    modelUrl: null,
    notes: 'Plan items-player-039 — Kupiec stock. Wearable arms armor; heaviest V1 pauldron, still far below chainmail cost.',
    armor: {
      slot: 'arms',
  ...
  ring_gold: {
    ...
    spawn: 'none',
    modelUrl: null,
    notes: 'Quaternius Ultimate RPG Pack `Ring1` — FBX2glTF → gltfpack -cc. Kupiec stock, sprzedawalna biżuteria bez mechaniki noszenia.',
  },
  ring_turquoise: {
    kind: 'ring_turquoise',
    ...
    spawn: 'none',
    modelUrl: null,
    notes: 'Quaternius Ultimate RPG Pack `Ring4`. Kupiec stock, sprzedawalna biżuteria bez mechaniki noszenia.',
  },
  ring_amethyst: {
    kind: 'ring_amethyst',
    ...
    spawn: 'none',
    modelUrl: null,
    notes: 'Quaternius Ultimate RPG Pack `Ring5`. Kupiec stock, sprzedawalna biżuteria bez mechaniki noszenia.',
  },
  bandit_ledger: {
    kind: 'bandit_ledger',
    ...
    spawn: 'none',
    modelUrl: null,
    notes: 'Plan quests-progression-026 — identity-backed ledger from the dungeon bandit deep stash. Not merchant stock.',
    physical: { resilient: true },
  },
  marked_valuable: {
  ...
  encoded_chronicle: {
    ...
    spawn: 'none',
    modelUrl: null,
    notes: 'Plan quests-progression-038 — identity-backed encoded chronicle. Not merchant stock.',
    physical: { resilient: true },
  },
  ...
  masterwork_hunting_bow: {
    ...
    holdable: true,
    melee: null,
    ranged: { damage: 24, range: 17, projectileSpeed: 32, drawTime: 0.38, recovery: 0.25, staminaCost: 7, accuracy: 0.9, ammoKinds: ['arrow', 'broadhead_arrow', 'war_arrow'], criticalChance: 0.1 },
    spawn: 'none',
    modelUrl: null,
    notes: 'Plan items-player-047 — quest/exceptional reward, not Kupiec stock (never MERCHANT_STOCK or PREMIUM_MERCHANT_KINDS). Fastest, most accurate bow; less raw damage than long_bow.',
  },
  arrow: {
    kind: 'arrow',
    label: 'strzała',
  ...
  flax: {
    ...
    spawn: 'world_chunk',
    modelUrl: null,
    notes: 'Plan settlements-npcs-007 — wild fibre (`terrain/chunkItems.ts` flora pool). Gathered into `Household.items`, processed by `textile_worker`.',
  },
  linen_material: {
    kind: 'linen_material',
    ...
    spawn: 'none',
    modelUrl: null,
    notes: 'Plan settlements-npcs-007 — textile output of `3 flax → 1 linen_material` via `executeProduction`. Household `items` only.',
  },
  poisonous_herb: {
    kind: 'poisonous_herb',
    label: 'trujące ziele',
    ...
    spawn: 'world_chunk',
    modelUrl: null,
    notes: 'Plan settlements-npcs-007 — distinct swamp/forest-floor collectible (`terrain/chunkItems.ts`). No consumable/poison effect in V1.',
  },
  dressing: {
    kind: 'dressing',
    ...
    spawn: 'none',
    modelUrl: null,
    notes: 'Plan settlements-npcs-007 — `herbalist` output of `1 bandage + 1 herb → 1 dressing`. Catalog-driven health consumable for npc-002 healing.',
    consumable: { need: 'health', relief: 50 },
  },
  book_riding_basic: {
