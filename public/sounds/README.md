...
| ambient-rain-storm-01.ogg | — | — (TBD — provided directly in `public/sounds/`, source/license not yet recorded) | Storm-only secondary bed via `AMBIENT_STORM_WIND_LOOP_URL` in `audio/weatherSounds.ts` (not used for plain `rain` or mountain ambient wind) |
...
| ui-click-03.ogg | interface1.wav | RPG Sound Pack (artisticdude / OpenGameArt; verify CC0) | temporary negative-consequence placeholder (plan items-player-042) |
